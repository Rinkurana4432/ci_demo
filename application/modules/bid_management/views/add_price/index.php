<div class="x_content">
               ...
               <div class="control-group">
                  <div class="controls">
                     <div class="input-prepend input-group">
                        ...
                        <?php /*  <input type="text" style="width: 200px" name="reservation" id="reservation" class="form-control" value="" data-table="bid_management/accounts"> */?>
   ...
   <div class="row hidde_cls export_div">
      <div class="col-md-12">
         <input type="hidden" value='account' id="table" data-msg="Company" data-path="bid_management/competitor_details"/>
        
         <div class="btn-group"  role="group" aria-label="Basic example">
		     <?php 
               if($can_add) {
               	//echo '<a href="'.base_url().'bid_management/editAccount/"><button type="buttton" class="btn btn-info">Add</button></a>'; 
               	echo '<button type="button" class="btn btn-success add_bid_mng_tabs addBtn" data-toggle="modal" id="add" data-id="add_price"><i class="fa fa-plus-circle valignmiddle btnTitle-icon"></i>Add Price</button>';
               } ?>
         ...
         <tbody>
            <?php if(!empty($competitor_details)){
               foreach($competitor_details as $competitordetails){ 
                   $action = '';
                   if($can_edit) { 
               		//$action = $action.'<a href="'.base_url().'bid_management/editAccount/'.$account["id"].'" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> Edit </a>'; 
               		$action =  '<a href="javascript:void(0)" id="'. $competitordetails["id"] . '" data-id="add_price" class="add_bid_mng_tabs btn btn-edit  btn-xs" data-tooltip="Edit" id="' . $competitordetails["id"] . '"><i class="fa fa-pencil"></i> </a>';
               	}	
               	/**if($can_view) { 				
               		$action .=  '<a href="javascript:void(0)" id="'. $competitordetails["id"] . '" data-id="add_price_view" class="add_bid_mng_tabs btn btn-view   btn-xs" data-tooltip="View" id="' . $competitordetails["id"] . '"><i class="fa fa-eye"></i>  </a>';			
               	}
               	**/
               
               	if($can_delete) { 
               		$action = $action.'<a href="javascript:void(0)" data-tooltip="Delete"  class="delete_listing btn btn-xs btn-delete" data-href="'.base_url().'bid_management/delete_add_price_compt/'.$competitordetails["id"].'"><i class="fa fa-trash"></i></a>';
               	}
               	...
               	$comptname = getNameById('bid_competitor_details',$competitordetails['account_id'],'id');
               	if(!empty($comptname)){
               ...
               <img class="img-responsive hidde" src="'.base_url(). 'assets/images/draft.png" > </a>';
                  echo "<td><input name='checkbox[]' class='checkbox1 checkbox[]' type='checkbox'  data-ai=".$competitordetails['id']." value=".$competitordetails['id'].">";
               						   		if($competitordetails['favourite_sts'] == '1'){
               						   				...
               						   				echo"<input type='hidden' value='account' id='favr' data-msg='Company' data-path='bid_management/accounts'  id-recd=".$competitordetails['id'].">";
               						   		}else{
