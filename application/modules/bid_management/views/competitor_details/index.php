<div class="x_content">
   ...
   <div class="col-md-12 col-xs-12 for-mobile">
      ...
      <form class="form-search" id="orderby" method="post" action="<?= base_url() ?>bid_management/competitor_details">
         <input type="hidden" name="order" id="order" value="<?php if(isset($_POST['order'])==''||$_POST['order']=='desc'){echo 'asc';}else{echo 'desc';}?>">
            ...
            <form action="<?php echo base_url(); ?>crm/competitor_details" method="post" id="date_range">	
               <input type="hidden" value='' class='start_date' name='start'/>
               <input type="hidden" value='' class='end_date' name='end'/>
            </form>
         </div>
                     <?php if(!empty($competitor_details) && $can_view){ ?>
                     <form action="<?php echo base_url(); ?>crm/competitor_details" method="post">
   ...
   <div class="stik">
      <form action="<?php echo site_url(); ?>crm/competitor_details" method="post" id="export-form">
         ...
         <div class="btn-group"  role="group" aria-label="Basic example">
		     <?php 
               if($can_add) {
               	...
               	echo '<button type="button" class="btn btn-success add_bid_mng_tabs addBtn" data-toggle="modal" id="add" data-id="competitor_details"><i class="fa fa-plus-circle valignmiddle btnTitle-icon"></i>Add</button>';
               } ?>
   ...
   <div id="print_div_content">
      ...
      <table id="mytable" class="table tblData table-striped table-bordered account_index" data-id="account" style="width:100%; margin-top:40px;" border="1" cellpadding="3">
         <thead>
            <tr>
               <th scope="col">All<br><input id="selecctall" type="checkbox"></th>
            ...
            <?php if(!empty($competitor_details)){
               foreach($competitor_details as $competitordetails){ 
                   ...
                   if($can_edit) { 
               		...
               		$action =  '<a href="javascript:void(0)" id="'. $competitordetails["id"] . '" data-id="competitor_details" class="add_bid_mng_tabs btn btn-edit  btn-xs" data-tooltip="Edit" id="' . $competitordetails["id"] . '"><i class="fa fa-pencil"></i> </a>';
               	}	
               	/*#if($can_view) { 				
               		$action .=  '<a href="javascript:void(0)" id="'. $competitordetails["id"] . '" data-id="account_view" class="add_bid_mng_tabs btn btn-view   btn-xs" data-tooltip="View" id="' . $competitordetails["id"] . '"><i class="fa fa-eye"></i>  </a>';			
               	#}*/
               	  if($can_view) { 
               		//$action = $action.'<a href="'.base_url().'crm/editAccount/'.$account["id"].'" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> Edit </a>'; 
               		$action =$action.'<a href="javascript:void(0)" id="'. $competitordetails["id"] . '"  data-id="view_competitor_details" data-tooltip="View" class="add_bid_mng_tabs btn btn-view  btn-xs"><i class="fa fa-eye"></i></a>';
               		}
               ...
               <img class="img-responsive hidde" src="'.base_url(). 'assets/images/draft.png" > </a>';
                  echo "<td><input name='checkbox[]' class='checkbox1 checkbox[]' type='checkbox'  data-ai=".$competitordetails['id']." value=".$competitordetails['id'].">";
               						   		if($competitordetails['favourite_sts'] == '1'){
               						   				...
               						   				echo"<input type='hidden' value='account' id='favr' data-msg='Company' data-path='crm/accounts'  id-recd=".$competitordetails['id'].">";
               						   		}else{
