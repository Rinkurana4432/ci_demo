...
<form method="post" class="form-horizontal" action="<?php echo base_url(); ?>bid_management/saveSaleOrder" enctype="multipart/form-data" id="leadForm" novalidate="novalidate">
		...
		<input type="hidden" name="revised_so_code" value="<?php if (!empty($sale_order)) { echo $revisedSOCode; } ?>" class="revised_po_code">
...
<div id="Material-Details" class="container tab-pane active">
	...
	<div class=" form-group input_productre middle-box" style="position: relative; clear: both;">	
		<?php if(empty($register_opportunity)){ ?>	
				...
				<div class="well scend-tr mobile-view"   id="chkIndex_1">				
					...
					<label  class="col-md-12">Material Name<span class="required">*</span></label>
						<?php /*<select class="materialNameId  form-control selectAjaxOption select2 Add_mat_onthe_spot" required="required" name="product[]" data-id="material" data-key="id" data-fieldname="material_name" data-where="created_by_cid = <?php echo $this->companyGroupId; ?> AND status = 1" width="100%" onchange="getTax(event,this)" id="material" required>
						*/?>
						<select class="materialNameId  form-control selectAjaxOption select2 Add_mat_onthe_spot" required="required" name="product[]"  width="100%" onchange="getTax(event,this)" id="material" required>
							<option value="">Select Option</option>
						</select>
		...
		<?php } else{ 
					$products = json_decode($register_opportunity->product_detail);
					//pre($products);
					if(!empty($products)){ 
						$i =  1;
						?>
						...
						?>
							<div class="well <?php if($i==1){ echo 'edit-row1 scend-tr mobile-view';}else{ echo 'scend-tr mobile-view';}?>"  id="chkWell_<?php echo $i; ?>" style="overflow:auto; ">
											...
											<select class="materialNameId  form-control selectAjaxOption select2 Add_mat_onthe_spot" required="required" name="product[]" data-id="material" data-key="id" data-fieldname="material_name" data-where="created_by_cid = <?php echo $this->companyGroupId; ?>" width="100%" onchange="getTax(event,this)" id="material">
												...
												<?php 
													...
													if(!empty($product)){
														...
														echo '<option value="'.$product->material_name_id.'" selected>'.$material->material_name.'</option>';
													}
												?>
											</select>
											<input type="hidden" name="mat_idd_name" id="matrial_Iddd">	
								...
								<div class="col-md-1 col-sm-6 col-xs-12 form-group">
								<label>	Qty	</label>
									<input type="text" name="qty[]"  placeholder="Qty" class="form-control col-md-7 col-xs-12 quantity" value="<?php echo $product->qty ;?>" onkeyup="keyupFunction(event,this)" onchange="keyupFunction(event,this)">
								</div>
								<div class="col-md-1 col-sm-6 col-xs-12 form-group">
								<label>	UOM	</label>
							...
							<input type="hidden" name="uom[]" readonly value="<?php echo  $product->uom_material; ?>">
								...
								<div class="col-md-2 col-sm-6 col-xs-12 form-group">
								<label>	Total	</label>
									<input type="text" name="totals[]" class="form-control col-md-7 col-xs-12 total" value="<?php echo $product->total ;?>" readonly>
								</div>	
								<div class="col-md-2 col-sm-6 col-xs-12 form-group">
								<label style="border-right: 1px solid #c1c1c1 !important;">	GST	Total</label>
									<input style="border-right: 1px solid #c1c1c1 !important;" type="text" name="TotalWithGsts[]" class="form-control col-md-7 col-xs-12 totalWithGst" value="<?php echo $product->TotalWithGst ;?>" readonly>
								</div>									
...
<div class="col-md-12 col-sm-12 col-xs-12" style="clear:both; margin-top:22px;">
					<div class="col-md-4 col-sm-5 col-xs-12 text-right" style="float: right;">
						...
						<div class="col-md-7 col-sm-5 col-xs-6 text-right">
						  <input type="hidden"  name="total" required="required" class="form-control col-md-7 col-xs-12"  value="<?php if(!empty($product)) echo $product->total; ?>"> 
							<strong>Total:</strong>&nbsp;&nbsp;
							</div>
							<div class="col-md-5 col-sm-5 col-xs-6 text-left">
							      <span class="fa fa-rupee divSubTotal"><?php if(!empty($product)){ echo $product->total ; } else{ echo 0; }?></span>
							</div>
							 
						</div>
						<div class="col-md-12 col-sm-12 col-xs-12 text-right" style="font-size: 18px;color: #2C3A61; border-top: 1px solid #2C3A61;">
						<div class="col-md-7 col-sm-5 col-xs-6 text-right">
						  <input type="hidden"  name="grandTotal" required="required" class="form-control col-md-7 col-xs-12"  value="<?php if(!empty($product)) echo $product->TotalWithGst; ?>"> 
							<strong>Grand Total:</strong>&nbsp;
							</div>
							<div class="col-md-5 col-sm-5 col-xs-6 text-left">
							     <span class="fa fa-rupee divTotal"><?php if(!empty($product)){ echo $product->TotalWithGst ; } else{ echo 0; }?></span>
							</div>
