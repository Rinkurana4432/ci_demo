...
<form class="form-search" method="get" action="<?= base_url() ?>hrm/emp_weekoff">
   <div class="col-md-6">
      <div class="input-group">
         <span class="input-group-addon">
         <i class="ace-icon fa fa-check"></i>
         </span>
         <input type="text" class="form-control search-query" placeholder="Enter Name" name="search" id="search" data-ctrl="hrm/emp_weekoff" value="<?php if(!empty($_GET['search']))echo $_GET['search'];?>">
         <span class="input-group-btn">
         <button type="submit" class="btn btn-purple btn-sm">
         <span class="ace-icon fa fa-search icon-on-right bigger-110"></span>
         Search
         </button>
		 <input type="button" name="submitSearchReset" class="btn btn-outline-secondary" onclick="window.location.href='<?= base_url() ?>hrm/emp_weekoff'" value="Reset">
         </span>
      </div>
   </div>
</form>
   ...
   <div class="col-md-12  export_div">
      <div class="col-md-4 col-sm-12 datePick-right"><?php if($can_add) {
         echo '<button type="button" class="btn btn-primary hrmTab addBtn" data-toggle="modal" id="add" data-id="weekoffEmpAdd">Add Weekoff</button>';
         } ?></div>
   </div>
   <table id="" class="table table-striped table-bordered account_index" cellspacing="0" width="100%" style="margin-top:45px;">
      <thead>
         <tr>
            <th scope="col">ID
			<span><a href="<?php echo base_url(); ?>hrm/emp_weekoff?sort=asc" class="up"></a>
		   <a href="<?php echo base_url(); ?>hrm/emp_weekoff?sort=desc" class="down"></a></span></th>
            <th scope="col">Name
			<span><a href="<?php echo base_url(); ?>hrm/emp_weekoff?sort=asc" class="up"></a>
		   <a href="<?php echo base_url(); ?>hrm/emp_weekoff?sort=desc" class="down"></a></span></th>
      ...
      <tbody>
         ...
         <tr>
            ...
            <td data-label="action:" class="jsgrid-align-center hidde action"><i class='fa fa-cog'></i><div class='on-hover-action'>
               <?php echo  '<a href="javascript:void(0)" id="'. $value->id . '" data-id="weekoffEmpAdd"  class="hrmTab btn btn-edit  btn-xs"  >Edit</a>'; ?>
               <?php  echo '<a href="javascript:void(0)"  class="delete_listing
                  btn btn-delete btn-xs" data-href="'.base_url().'hrm/deleteempweekoffHoliday/'.$value->id.'" >Delete</a>';?>
				</div>
            </td>
         </tr>
         <?php endforeach; ?>
