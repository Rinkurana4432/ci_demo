                                           ...
                                           <?php foreach($attendancelist as $value):  
                                               $empDatales = getNameById('worker',$value['emp_id'],'id'); 
                                                ...
                                                $shiftNameTime = getNameById('production_setting',$employeeshift['shift_id'],'id');
                                                  if((float)$value['working_hour']>(float)$shiftNameTime->working_hrs){
                                                $overTime=(float)$value['working_hour']-(float)$shiftNameTime->working_hrs;
                                                       }
                                                    }?>
                                            <tr>
                                                ...
                                                <td><?php if((float)$value['working_hour']>(float)$shiftNameTime->working_hrs){ 
                                                            echo   $overTime;
                                                 } else { echo'0'; }?></td>
