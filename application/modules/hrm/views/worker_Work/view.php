...
<div    class="col-md-12 col-xs-12 for-mobile">
 <div class="Filter Filter-btn2">
    ...
    <a href="<?php echo base_url(); ?>hrm/monthly_salary_report/"><input type="button" name="submitSearchReset" class="btn btn-outline-secondary" value="Reset"></a>
               ...
               <div class="control-group">
                  <div class="controls">
                     <div class="input-prepend input-group">
                        ...
                        <input type="text" style="width: 200px" name="tabbingFilters" id="tabbingFilters" class="form-control" value=""  data-table="hrm/monthly_salary_report"/>
                     </div>
                  </div>
               </div>
            </fieldset>
            <form action="<?php echo base_url(); ?>hrm/monthly_salary_report " method="get" id="date_range">   
   ...
   <table id="" class="table table-striped table-bordered account_index" border="1" cellpadding="3" data-order='[[1,"desc"]]'>
      <thead>
         <tr>
            ...
            <th scope="col">OT <span>( Over Time )</span></th>
            <th scope="col">Wages Salary </th> 
      ...
      <tr>
         <td><?=$i;?></td>
         <td><form action="<?=base_url();?>hrm/worker_Work" > <input type="hidden" name="start" value="<?php if(!empty($start)){echo $start;}else{echo 'a';}  ?>"><input type="hidden" name="end" value="<?php if(!empty($end)){echo $end;}else{echo 'b';}?>"><input type="hidden" name="user_id" value="<?=$workerid;?>"><button><?=@$workeridgfr->name;?></button></form> </td>
         <td><?=@$workeridgfr->salary;?></td>
          <td><?php $wohrs12=0; foreach ($workernamevalue as  $wagesvalue) { if ($wagesvalue['wages_or_per_piece']=='wages') {
             $worker_id=json_decode($wagesvalue['worker_id']); 
            $workerhrs=json_decode($wagesvalue['working_hrs']); 
            foreach ($worker_id as $Wkey => $Wvalue) {
               if ($workerid==$Wvalue) { 
              foreach ($workerhrs as $Hrskey => $Hrsvalue) {
                if ($Wkey== $Hrskey) {
                      if ($Hrsvalue>$worker_hours) { 
                        $othrs=$Hrsvalue-$worker_hours;
                         $wohrs12 +=$othrs;
                      } 
                    }
                }
          ...
          echo  $wohrs12.' '.'Hrs';
           ?></td>
          <td><?php $wagessalary=0;foreach ($workernamevalue as  $wagesvalue) {
          ...
          }   @$wagessallery12 +=$wagessalary;
         echo $amount = money_format('%!i', $wagessalary);?></td>
          <td><?php $persalary=0;foreach ($workernamevalue as  $pervalue) {
             if($pervalue['wages_or_per_piece']=='per_piece'){
         ...
         <td><?php  $total=$amountper + $amount;
                     $overallsallery12 +=$total;
       ...
       <?php $i++; 
         ...
         @$output[] = array( 
                     ...
                     'Over Time' =>  $hjdgjh=  money_format('%!i', $wohrs12).' '.'Hrs',
                     'Wages Salary ' =>$amount = money_format('%!i', $wagessalary), 
                     'Per-Piece  Salary' =>$amountper = money_format('%!i', $persalary),  
                     'Total' => $ggh=money_format('%!i', $total),
                  );      
