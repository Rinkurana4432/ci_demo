         ...
         <div class="btn-group"  role="group" aria-label="Basic example">
            ...
            <div class="dropdown btn btn-default buttons-copy buttons-html5 btn-sm export ">
               ...
               <ul class="dropdown-menu" role="menu" id="export-menu">
				  ...
				  <li id="export-to-blank-excel"><a href="<?php echo base_url()?>assets/modules/hrm/download/workerexcel.xlsx" title="Please check your open office Setting">Export to Blank Excel</a></li>
               </ul>
            </div>
		...
		<div class="row hidde_cls filter1 progress_filter">
           ...
           <div class="dropdown btn btn-default buttons-copy buttons-html5 btn-sm export ">
               <ul class="dropdown-menu import-bar" role="menu" id="export-menu">
                  <form action="<?php echo base_url(); ?>hrm/importworker" method="post" enctype="multipart/form-data">
                  ...
                  <tbody>
                     ...
                     <tr>
                        ...
                        <td><?php if($worker['worker_type']=="on_roll"){
                           ...
                           echo "Temporary"; 
                           }elseif($worker['worker_type']=="worker"){
                           echo "Worker"; 
                        ...
                        <td class='hidde action'>
						 <i class='fa fa-cog'></i>
                           ...
                           <?php 
                              ...
                              if($can_edit)
                              	echo '<a href="javascript:void(0)"  class="btn btn-edit btn-xs hrmTab" data-toggle="modal" data-id="workerEdit" id="'.$worker["id"].'">Edit</a>'; 
                              if($can_view) 
                              	echo '<a href="javascript:void(0)"  class="btn btn-view btn-xs hrmTab" ata-toggle="modal" data-id="workerView" id="'.$worker["id"].'">View</a>';
                              if($can_delete && $worker["used_status"]==1){
                              	echo '<a href="javascript:void(0)"  class="
                              	btn btn-delete btn-xs" data-href="'.base_url().'hrm/deleteWorker/'.$worker["id"].'" disabled="disabled">Delete</a>';
                              }elseif($can_delete){
                              	echo '<a href="javascript:void(0)"  class="delete_listing
                              	btn btn-delete btn-xs" data-href="'.base_url().'hrm/deleteWorker/'.$worker["id"].'" >Delete</a>';
                              }
                     ...
                     <?php
					  if($worker['salarySlab'] != 0 ){
						 $salaryslabData = getNameById('salary_slab',$worker['salarySlab'],'id');
						 $datae = json_decode($salaryslabData->slab_structure);
						 $basic= $worker['salary']*$datae->basic/100;
                   $hra= $worker['salary']*$datae->hra/100;
                   $sa= $worker['salary']*$datae->sa/100;
                   $ca= $worker['salary']*$datae->ca/100;
                   $da= $worker['salary']*$datae->da/100;
                   $incentive= $worker['salary']*$datae->incentive/100;
                   $esic= $worker['salary']*$datae->esic/100;
                   $medical= $worker['salary']*$datae->medical/100;
                   $tds= $worker['salary']*$datae->tds/100;
                   $output[] = array(
                        'Emp Code'=>$worker['id'],
                        'Emp Name' => $worker['name'],
                        'Mobile number' => $worker['mobile_number'],
                        'Emp Father Name' => $worker['fathername'],
                        'Emp Deparment' => $worker['department'],
                        'Emp Type' => $worker['worker_type'],
                        'Emp Designation' => $worker['designation'],
                        'Emp Date of Birth' => $worker['date_of_birth'],
                        'Emp Education' => $worker['education'],
                        'Emp Plant Location' => $worker['plantLocation'],
                        'Emp Address' => $worker['address'],
                        'State' => $state->state_name,
                        'City' =>  $city->city_name,
                        'Date of Joining' => $worker['date_of_joining'],
                        'Aadhar No' => $worker['adharNo'],
                        'PAN No' => $worker['panNo'],
                        'A/c No' => $worker['account_no'], 
					...
					$output[] = array(
                        ...
                        'Basic' => $worker['Basic'],
                        'HRA' => $worker['HRA'],
                        'DA' =>  $worker['Dearness_Allowance'],
                        'SA' =>  $worker['Special_Allowance'],
                        'CA' =>  $worker['Conveyance_Allowance'], 
                        'Created Date' => date("d-m-Y", strtotime($worker['created_date'])),
                         );	
                     }    
                    }
                     ...
                     <tr>
                        ...
                        <td class='hidde action'>
                           ...
                           <?php 
                              ...
                              if($can_edit)
                              	echo '<a href="javascript:void(0)" data-tooltip="Edit" class="btn btn-edit btn-xs hrmTab" data-toggle="modal" data-id="workerEdit" id="'.$inactive["id"].'">Edit</a>'; 
                              if($can_view) 
                              	echo '<a href="javascript:void(0)" data-tooltip="View" class="btn btn-view btn-xs hrmTab" ata-toggle="modal" data-id="workerView" id="'.$inactive["id"].'">View  </a>';
                              if($can_delete && $inactive["used_status"]==1){
                              	echo '<a href="javascript:void(0)" data-tooltip="Delete" class="
                              	btn btn-delete btn-xs" data-href="'.base_url().'hrm/deleteWorker/'.$inactive["id"].'" disabled="disabled">Delete</a>';
                              }elseif($can_delete){
                              	echo '<a href="javascript:void(0)" data-tooltip="Delete" class="delete_listing
                              	btn btn-delete btn-xs" data-href="'.base_url().'hrm/deleteWorker/'.$inactive["id"].'" >Delete</a>';
                              }
                              ?>
							</div>
                        </td>
