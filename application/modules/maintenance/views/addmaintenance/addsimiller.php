<form method="post" class="form-horizontal" action="<?php echo base_url(); ?>maintenance/savebreakdown" enctype="multipart/form-data" id="similarmachine" novalidate="novalidate">
   ...
   <input type="hidden" name="acknowledge" value="<?php echo $addsimilerdata->acknowledge; ?>" id="acknowledge"> 
   <input type="hidden" name="assign_worker" value="<?php echo $addsimilerdata->assign_worker; ?>" id="assign_worker"> 
   <input type="hidden" name="aknowlwdge_by" value="<?php echo $addsimilerdata->aknowlwdge_by; ?>" id="aknowlwdge_by"> 
   <input type="hidden" name="requested_by" value="<?php echo $addsimilerdata->requested_by; ?>" id="requested_by">  
   <div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
      <div class="item form-group">
         ...
         <div class="col-md-6 col-sm-12 col-xs-12">
		 	<select class="form-control selectAjaxOption select2 select2-hidden-accessible get_machine_id_select select2" required="required" name="machine_id" data-id="add_machine" data-key="id" data-fieldname="machine_name" data-where="created_by_cid='<?php echo $_SESSION['loggedInUser']->c_id; ?>'" tabindex="-1" aria-hidden="true" >
			   ...
			   <?php 
					if(!empty($addsimilerdata->machine_id)){
						$machine = getNameById('add_machine',$addsimilerdata->machine_id,'id');
      ...
      <div class="item form-group">
         ...
         <div class="col-md-6 col-sm-12 col-xs-12">
            ...
            <input type="radio" value="electronic" name="machine_type" <?php if($addsimilerdata->machine_type =='electronic' ){ echo "checked"; } ?> >Electronic
            </label>
            <label class="radio-inline">
            <input type="radio" value="machnical" <?php if($addsimilerdata->machine_type =='machnical' ){ echo "checked"; } ?> name="machine_type">Machnical
            </label>
            <label class="radio-inline">
            <input type="radio" value="electronic & machnical" <?php if($addsimilerdata->machine_type =='electronic & machnical' ){ echo "checked"; } ?> name="machine_type">Both
            </label> 				
         </div>
      </div>
   </div>
   <div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
      <div class="item form-group">
         ...
         <div class="col-md-6 col-sm-12 col-xs-12">
            <input id="breakdown_couses" class="form-control col-md-7 col-xs-12" name="breakdown_couses" placeholder="Breakdown Couses" required="required" type="text" value="<?php echo $addsimilerdata->breakdown_couses; ?>"> 				
         </div>
      </div>
      <div class="item form-group">
         ...
         <div class="col-md-6 col-sm-12 col-xs-12">
            <select class="form-control" name="priority">
               <option value="high" <?php if($addsimilerdata->priority =='high'){ echo "selected"; } ?> >High</option>
               <option value="medium" <?php if($addsimilerdata->priority =='medium'){ echo "selected"; } ?> >Medium</option>
               <option value="low" <?php if($addsimilerdata->priority =='low'){ echo "selected"; } ?> >Low</option>
            </select>
         </div>
      </div>
   </div>
      ...
      <div class="item form-group">
         ...
         <div class="col-md-6 col-sm-12 col-xs-12">
            <select class="form-control selectAjaxOption select2 select2-hidden-accessible assign_worker select2" required="required" name="assign_worker" data-id="worker" data-key="id" data-fieldname="name" data-where="created_by_cid='<?php echo $_SESSION['loggedInUser']->c_id; ?>' AND worker_type='maintenance'" tabindex="-1" aria-hidden="true" >
               <option value="">Select option</option>
			   		<?php 
						if(!empty($addsimilerdata->assign_worker)){
						$workers = getNameById('worker',$addsimilerdata->assign_worker,'id');
