        ...
        foreach ($CombinedArray as $combinedValue) {
           # pre($combinedValue);
            ...
            foreach ($combinedValue['material'] as $key => $mat_detail) {
                // if($mat_detail['closing_balance'] == 0){
					//pre($mat_detail);
                ...
                if ($sum > 0 || $sum < 0) {
					...
					$resvQty = 0;
					 $AvlbQty = 0;
                    $rsv = getNameById_mat('reserved_material',$mat_detail['material_name_id'],'mayerial_id');
					 $AvlbQty = get_available_mat_quantity($mat_detail['material_name_id']);
                   if(!empty($rsv)){
                        foreach($rsv as $rv){
                            $resvQty += $rv['quantity'];
                            // pre($rv['quantity']);
						     ...
						     <h6>See all locations</h6>
                               <div class=' see-all-location '>						
                                ...
                                <tbody>";
                                    $n = 0;
									$sameMatdata = [];
									foreach($mat_detail['location'] as $locationData1){
										$sameMatdata[$locationData1['location']][]= $locationData1;
										
									}
                                    foreach ($sameMatdata as $locationkey1 => $locationData) {
										$totalQty = 0;
										foreach($locationData as $getQty){
											$totalQty += $getQty['qty'];
											// $uomdata = $getQty['qty'];
										}
										// pre($locationData);
										
                                        $ww = getNameById('uom', $locationData[0]['Qtyuom'], 'id');
                                        $uom = !empty($ww) ? $ww->ugc_code : '';
                                        $locationName = getNameById('company_address', $locationkey1, 'id');
                                        ...
                                        echo "<tr class='locRow' id='chkIndex_" . $n . "'>
                                                ...
                                                <td class=''>" . $totalQty . "<input type='hidden' class='locQty' value='" . $totalQty . "'</td>
                                                <td>" . $uom . "</td>
...
?>
                            ...
                            <?php
                              echo  '<div class="view-buttonshover"><a target="_BLANK"href="'.base_url().'inventory/inventory_adjustmentListing_view?id='.$mat_detail['material_name_id'].'" class="
                                                 ...
                                                 // echo  '<a target="_BLANK"href="'.base_url().'inventory/view_lot_report?id='.$mat_detail['material_name_id'].'" class="btn btn-delete btn-xs" data-href="" >View Lot</a>';
                     ...
                     ?> >
                                                ...
                                                ?>" data-uom ="<?php echo $mat_detail['uom']; ?>" style="display:none;">Half/Full Book</a>
