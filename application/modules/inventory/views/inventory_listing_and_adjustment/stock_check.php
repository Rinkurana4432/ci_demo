	...
	<input type="hidden" name="id" value="<?php if($inventoryListing && !empty($inventoryListing)){ echo $inventoryListing->id; }?>">
		...
		<input type="hidden" name="material_type_id" value="" id="material_type_id">
			...
			<div class="item form-group">
					...
					<input type="hidden" id="Actiontype" name="action_type" required="required" class="form-control col-md-7 col-xs-12"  value="" readonly>
					</div>
			</div>	
			
			<div class="item form-group">
				<div style="font-size:20px;" class="mat_name">Material Name:<b><span  id="material_name" style="color:green;"></span></b></div>
			</div>	
			<div class="item form-group">
			<label class="control-label col-md-3 col-sm-3 col-xs-12" for="mat_name">Date</label>
				<div class="col-md-6 col-sm-6 col-xs-12">
								...
								<table id="datatable-buttons" class="table table-striped table-bordered jambo_table bulk_action"  data-id="user">
									...
									if(!empty($inventoryListing) && $inventoryListing->location !=''){ 
										...
										<td>
											...
											<input type="hidden" id="address" name="source_location[]" required="required" class="form-control col-md-7 col-xs-12" placeholder="address" value="<?php  if(!empty($locationName)) {echo $locationName->location; } else{ echo "N/A" ;} ?>">
												<span><?php if(!empty($locationName)) { echo $locationName->location; } else{ echo "N/A" ;} ?></span>
											</div>
										</td>
										<!--td>
											<div class="col-md-4 col-sm-6 col-xs-12">
											<input type="hidden" id="area" name="source_storage[]" required="required" class="form-control col-md-7 col-xs-12" placeholder="area" value="<?php //if(!empty($source_Data)) echo $source_Data->Storage; ?>">
											<?php //if(!empty($source_Data)) echo $source_Data->Storage; ?>
											</div>
										</td-->
										<td>
											<div class="col-md-4 col-sm-6 col-xs-12">
											<input type="hidden" id="rack_number" name="source_rack_no[]" required="required" class="form-control col-md-7 col-xs-12" placeholder="rack_number" value="<?php if(!empty($source_Data)) echo $source_Data->RackNumber; ?>">
			...
			<div class="item form-group">
				<label class="control-label col-md-3 col-sm-3 col-xs-12" for="mat_name">Total Value</label>
					<div class="col-md-6 col-sm-6 col-xs-12">
			...
			<div class="item form-group">
				<label class="control-label col-md-3 col-sm-3 col-xs-12" for="mat_name">Stock check value</label>
