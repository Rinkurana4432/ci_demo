      ...
      <table id="" class="table table-striped table-bordered account_index" data-id="account" style="margin-top:40px;">
         <thead>
            <tr>
               ...
               <th scope="col">Material Type<span><a href="<?php echo base_url(); ?>inventory/reorder_level?sort=asc" class="up"></a>
                  <a href="<?php echo base_url(); ?>inventory/reorder_level?sort=desc" class="down"></a></span>
               </th>
         ...
         <tbody>
            <?php if (!empty($reorder_level)) {
               ...
               foreach ($reorder_level as $reorderlevel) {
                    $inventory_items= json_decode($reorderlevel['inventory_items']);
                    $name_mat='';
                    foreach ($inventory_items as $key => $value) {
                       $mattyp = getNameById_mat('material_type', $value->type, 'id');
                        foreach ( $mattyp as $kesy => $name) { 
                                $name_mat = $name['name'];
                        }
                    }
               	   ...
               	   echo "<tr>
               				...
               				<td data-label='Report Date:'>" . date('d-m-Y',strTotime($reorderlevel['report_date'])) . "</td>
                            <td data-label='Report Date:'>" . $name_mat . "</td>
