...
<div id="print_div_content">  	<table  id="example" class="table table-striped table-bordered account_index" border="1" cellpadding="3" data-order='[[1,"desc"]]'>
	...
	<?php if(($can_validate==1) && ($data1['cause']!=''))
	{
		$disable='';
	}else{
		$disable='disabled';
	}
	$validatedBy = (!empty($data1['validated_by'])!=0)?(
	getNameById('user_detail',$data1['validated_by'],$data1['cust_id'])):''; 
	...
	<?php 
		...
		<p class='validatedBy'><?php echo "Validated By:";?><?php echo $validatedByName;?></p>
	<?php } ?>
	</td>
		     <td>
	...
	<button type="button" data-id="ViewNcr" data-tooltip="View" class="btn btn-edit btn-xs qualityTab" data-toggle="modal" id="<?php echo $data1['id']; ?>">View</button>						</td>
		</tr>
		<?php } ?>
		 </tbody>                               	
		 </table>
	...
	<?php if(($can_validate==1) && ($data['cause']!=''))
	{
		$disable='';
	}else{
		$disable='disabled';
	}
	$validatedBy = (!empty($data['validated_by'])!=0)?(
	getNameById('user_detail',$data['validated_by'],$data['cust_id'])):''; 
		$validatedByName = (!empty($validatedBy))?$validatedBy['name']:'';
	$selectApprove = $data['approve']==1?'checked':'';
	$selectDisapprove = $data['disapprove']==1?'checked':'';
	if($selectApprove =='checked'){
	echo "Approve:";?>
		<input type='radio' class='validate' data-idd='<?php echo $data['id'];?>' name='status_<?php echo $data1['id'];?>' value='Approve'/<?php echo $selectApprove.$disable;?>> 
		<?php echo "Disapprove:";?>
		<input type='radio' class='disapprove' data-idd='<?php echo $data['id'];?>' name='status_<?php echo $data1['id'];?>' value='Disapprove'/<?php echo $selectDisapprove.$disable;?> disabled>
		<p class='disapprove_reason'><?php echo $data['disapprove_reason'];?></p>
		<p class='validatedBy'>Validated By:<?php echo $validatedByName;?></p>
	<?php 
	}else{
