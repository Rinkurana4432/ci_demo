...
<form method="post" class="form-horizontal grnFormPurchase" enctype="multipart/form-data" id="mrn-form" novalidate="novalidate" action="<?php echo base_url(); ?>purchase/saveMRN">
   <?php
       // pre($mrnOrder);
       ?>
   <input type="hidden" name="id" value="<?php if ($mrn && !empty($mrn)) { echo $mrn->id;} ?>">
   <input type="hidden" name="po_id" value="<?php if ($mrnOrder && !empty($mrnOrder)) { echo $mrnOrder->id;}elseif($mrn && !empty($mrn)) { echo $mrn->po_id; } ?>">
   <input type="hidden" name="pi_id" value="<?php if ($mrnOrder && !empty($mrnOrder)) { echo $mrnOrder->pi_id;}elseif($mrn && !empty($mrn)) { echo $mrn->pi_id; } ?>">
   <?php
   ...
   <input type="hidden" name="convert_po_to_mrn" value="convert_po_to_mrn" >
   <div class="item form-group">
      ...
      <center><b>Order Created Date : </b> <?php echo $newDate; ?> </center>
      <br />
      <?php  }
         ?>
      ...
      <div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
         <div class="item form-group">
            ...
            <div class="col-md-6 col-sm-12 col-xs-12">
               <input type="text" name="bill_no" class="form-control col-md-7 col-xs-12" placeholder="Invoice No" value="<?= $gateEnteryData[0]['invoice_no']??'' ?>" required="required">
            </div>
         </div>
         <div class="item form-group">
         ...
         <div class="item form-group" style="display:none;">
            ...
            <div class="col-md-6 col-sm-12 col-xs-12">
               ...
               name="cost_center" data-id="purchase_cost_center" data-key="id" data-fieldname="cost_center_name" tabindex="-1" aria-hidden="true" data-where="created_by_cid=<?= $_SESSION['loggedInUser']->c_id ?> AND status=1" placeholder="Select Cost Center">
                  ...
                  <?php
                     if(!empty($mrnOrder)){
                        if( isset($mrnOrder->cost_center) ){
                           $costName = getNameById('purchase_cost_center',$mrnOrder->cost_center,'id');
      ...
      <div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
                  ...
                  <?php
                     if( $gateEnteryData[0]['supplier'] ){
                     	...
                     	if (!empty($mrnOrder)) {
                     		...
                     		echo '<option value="' . $mrnOrder->supplier_name . '" selected  data-id="' . $supplier_name_id->address . '">' . $supplier_name_id->name . '</option>';
                     	}	
                     }
         ...
         <div class="item form-group">
            ...
            <div class="col-md-6 col-sm-12 col-xs-12">
               <textarea id="address" name="address" class="form-control col-md-7 col-xs-12" placeholder="Display when supplier is selected from above"><?php 
                  if ($mrnOrder  && !empty($mrnOrder)) {
                  	$supplier_name_id = getNameById('supplier', $mrnOrder->supplier_name, 'id');
                  	echo $supplier_name_id->address;
                  } ?>
               </textarea>
            </div>
         </div>
                     ...
                     <div class="col-md-12 input_material middle-box for-bdr">
                        ...
                        <?php
                           foreach ($OrderMaterialDetails as $OrderMaterialDetail) {
                                    // pre($OrderMaterialDetail ); die;
                           	if($OrderMaterialDetail->remaning_qty !=0 ){
                           	// if($OrderMaterialDetail->remaning_qty !=0 || !isset($OrderMaterialDetail->remaning_qty)){
                           	$material_id = $OrderMaterialDetail->material_name_id;
                           	$materialName = getNameById('material', $material_id, 'id');
                              $allData = ['mrnOrder' => $mrnOrder,'OrderMaterialDetail' => $OrderMaterialDetail,'materialName' => $materialName,'i' => $i,'typeGrn' => 'convert_po_to_grn' ];
                                
                              /* grn material view in materialData */
                      //pre($allData);
                        ...
                        <?php	}
                           ...
                           ?>
                        <div class="col-sm-12 btn-row"><button class="btn  plus-btn addButton edit-end-btn" type="button" disabled>Add</button></div>
                     </div>
                  </div>
               </div>
            </div>
      ...
      <div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
         ...
         <div class="item form-group">
            <label class=" col-md-3 col-sm-3 col-xs-12" for="payment">Payment Mode</label>
            <div class="col-md-6 col-sm-12 col-xs-12">
         ...
         <div class="item form-group">
            ...
            <div class="col-md-6 col-sm-12 col-xs-12">
               <select class="form-control  address get_state_id select2-width-imp" name="delivery_address"  id="address" required="required">
                  <option value="">Select Option</option>
                  <?php
                     if (!empty($mrnOrder)) {
                     	$brnch_name = getNameById_with_cid('company_address', $mrnOrder->delivery_address, 'id','created_by_cid',$this->companyGroupId);
                                   	echo '<option value="' . $mrnOrder->delivery_address . '" selected   data-id="' . $mrnOrder->delivery_address . '">' . $brnch_name->location . '</option>';
                     }
                     ?>
               </select>
            </div>
         ...
         <div class="item form-group">
            <label class="col-md-5 col-sm-12 col-xs-12" for="textarea">Purchase Po to GRN Conversation Complete</label>
      ...
      <div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
         <div class="item form-group" id="freight1" >
            ...
            <div class="col-md-6 col-sm-12 col-xs-12">
               <input type="number" id="freight" name="freight" class="form-control col-md-7 col-xs-12 key-up-event" placeholder="Freight" <?php //if (!empty($mrnOrder)) echo 'readonly'; ?> value="<?php if (!empty($mrnOrder)) {echo $mrnOrder->freight;} ?>" onkeyup="keyupFunction(event,this)">
            </div>
         </div>
   <div class="item form-group" id="other_charges">
      ...
      <div class="col-md-6 col-sm-12 col-xs-12">
         <input type="number" id="other_charges" name="other_charges" class="form-control col-md-7 col-xs-12 key-up-event" placeholder="Other Charges" value="<?php if (!empty($mrnOrder)) {echo $mrnOrder->other_charges;} ?>" onkeyup="keyupFunction(event,this)" min="0" onkeypress="return float_validation(event, this.value)">
      </div>
   </div>
   <div class="item form-group">
      <label class="col-md-3 col-sm-12 col-xs-12" for="textarea">Terms and conditions </label>
      <div class="col-md-6 col-sm-12 col-xs-12">
         <textarea id="tnc" name="terms_conditions" class="form-control col-md-7 col-xs-12" placeholder="Terms And conditons" <?php if (!empty($mrnOrder)) echo 'readonly'; ?>><?php if (!empty($mrnOrder)) {
   ...
   <div class="item form-group">
    ...
    <div class="item form-group">
		...
		<div class="col-md-6 col-sm-12 col-xs-12">
		   <input type="text" id="current_date_2" name="date" class="form-control col-md-7 col-xs-12" placeholder="Display the Current Date" <?php if (!empty($mrnOrder)) echo 'readonly'; ?> value="<?php if (!empty($mrnOrder)) {	 echo date("d-m-Y", strtotime($mrnOrder->date));	} ?>"> 
		</div>
	 </div>
