...
<div class="x_content">
   <div class="row hidde_cls stik">
      ...
      <div class="Filter Filter-btn2">
         <form class="form-search" method="GET" action="<?php echo base_url(); ?>purchase/purchase_report/purchase_order_display">
            <div class="col-md-6">
               <div class="input-group">
                   <span class="input-group-addon">
                  <i class="ace-icon fa fa-check"></i>
                  </span>
                  <input type="text" class="form-control search-query" placeholder="Order No,Supplier Code" data-ctrl="purchase/purchase_report/purchase_order_display" name="search" id="search" value="<?php if(!empty($_GET['search']))echo $_GET['search'];?>">
                  <span class="input-group-btn">
                  <button type="submit" class="btn btn-sm">
                  <span class="ace-icon fa fa-search icon-on-right bigger-110"></span>
                  Search
                  </button>
                  <a href="<?php echo base_url(); ?>purchase/purchase_report/purchase_order_display"><input type="button" name="submitSearchReset" class="btn btn-outline-secondary" value="Reset"></a>
               </div>
            </div>
         </form>
         ...
         <div id="demo" class="collapse" aria-expanded="false" style="height: 2px;">
            <div class="col-md-12  col-xs-12 col-sm-12 datePick-left">
               ...
               <form action="<?= base_url() ?>purchase/purchase_report/purchase_order_display" method="GET" >
            ...
            <div id="print_div_content">
               <div class="table-responsive">
                  <table id="" style="width:100%;" class="table  table-bordered inprocess_div" data-id="user"  border="1" cellpadding="2" >
                        <?= table_th(['width:150px;' => 'PO#','text-align:center' => 'Supplier Code','Expected Date','Material Code','Comments','Total Qty','Cost Price']); ?>
                        <tbody>
                           <?php 
                              if( $poAnalysis ){
                                     ...
                                     ?>
                                       <tr>
                                          <td class="poOrderCode tdTextCenter">
                                             <span>
                                                <a href="javascript:void(0)" class="add_purchase_tabs" data-id="OrderView" id="<?= $grnValue['poId'] ?>"><?= $grnValue['poNo'] ?></a>
                                             </span>
                                          </td>
                                          ...
                                          <td><?= $grnValue['expected_delivery_date']??'N/A' ?></td>
                                          <td class="materialCode tdTextCenter">
                                             <span>
                                                <a href="javascript:void(0)" id="<?= $materialValue['material_name_id'] ?>" data-id="material_view" class="inventory_tabs">
                                                   <?= $materialValue['material_code'] ?>
                                                </a>
                                             </span>
                                          </td>
                                          <td><?= $grnValue['mrnComment']??'N/A' ?></td>
                                          <td><?= $materialValue['total_qty'] ?></td>
                                          <td>
                                             <?php 
                                                echo ($materialValue['cost_price'] * $materialValue['qty_delivered'] );
