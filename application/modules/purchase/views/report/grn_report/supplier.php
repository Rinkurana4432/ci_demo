            ...
            <div id="print_div_content">
               <div class="table-responsive">
                  <table id="" style="width:100%;" class="table  table-bordered inprocess_div" data-id="user"  border="1" cellpadding="2" >
                        <?= table_th(['Supplier Name','Supplier Code','Total Amount','Total Order']); ?>
                        <tbody>
                           <?php 
                              if( $grnSupplier ){
                                 foreach ($grnSupplier as $sDataKey => $sDateValue) { ?>
                                    <!--  Wrong amount is displaying Start 10-03-2022  -->
                                 <?php
                                    $materialNameData = json_decode($sDateValue['material_name']);
                                    $materialTotalAmount = 0;
                                    foreach ($materialNameData as $materialNameDataValue) {
                                       //$materialTotalAmount = $materialNameDataValue->debitNoteAmount;
                                       $materialTotalAmount += $materialNameDataValue->debitNoteAmount;
                                    }
                                    ?>
                                    <!--  Wrong amount is displaying End 10-03-2022  -->
                                       <tr>
                                          <td><?= $sDateValue['supplier_name'] ?></td>
                                          <td>
                                             <a href="javascript:void(0)" class="add_purchase_tabs" ata-toggle="modal" data-id="SupplierView" id="<?= $sDateValue['spId'] ?>">
                                                <?= $sDateValue['supplier_code'] ?>
                                             </a>
                                             </td>
                                           <!--  Wrong amount is displaying Start 10-03-2022  -->
                                           <td><?= $materialTotalAmount; ?></td>
                                          <!--  Wrong amount is displaying End 10-03-2022  -->
                                          <td><?= $sDateValue['total_order'] ?></td>
                                       </tr>
                              <?php      
                                 }
                              }
