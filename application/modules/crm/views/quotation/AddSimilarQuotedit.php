...
<form method="post" class="form-horizontal" action="<?php echo base_url(); ?>crm/savesimilarQuot" enctype="multipart/form-data" id="leadForm" novalidate="novalidate">
<input type="hidden" name="id" value="<?php if(!empty($quot)){ echo $quot->id; }?>">
	      ...
	      <label class="col-md-3 col-sm-12 col-xs-12" for="parent_account">Customer Name<span class="required">*</span></label>
							<div class="col-md-6 col-sm-12 col-xs-12">
								<select class="customerName selectAjaxOption select2 form-control" name="account_id" data-id="account" data-key="id" data-fieldname="name" width="100%" id="account_id" data-where="account_owner = <?php 	echo $this->companyGroupId; ?> AND save_status = 1" id="account_id" required="required">
									...
									<?php 
										if(!empty($quot)){
											$account = getNameById('account',$quot->account_id,'id');
	   	    ...
	   	    <div class="item form-group">
			       <div class="item form-group ">
							...
							<div class="col-md-6 col-sm-12 col-xs-12">
								<?php if(!empty($quot) && !empty($account)){
	...
	<div class="col-md-6 col-sm-12 col-xs-12 item form-group vertical-border">
       <div class="item form-group">	
		...
		<div class="col-md-6 col-sm-12 col-xs-12">
			<input type="text" class="form-control has-feedback-left datePicker" name="order_date" id="order_date" value="<?php if(!empty($quot) && $quot->order_date!=''){ echo $quot->order_date; }else {echo date("d-m-Y");}?>" required="required">
	...
	<div class=" item form-group ">													
		...
		<div class="col-md-6 col-sm-12 col-xs-12">
			<input type="text" class="form-control has-feedback-left datePicker" name="valid_date" id="valid_date" value="<?php if(!empty($quot) && $quot->valid_date!=''){ echo $quot->valid_date; }else {echo date("d-m-Y");}?>" required="required">
			<span class="fa fa-calendar-o form-control-feedback left" aria-hidden="true"></span>
			<span id="inputSuccess2Status3" class="sr-only">(success)</span>
		</div>						
	</div>
...
<div class=" item form-group ">													
		...
		<div class="col-md-6 col-sm-12 col-xs-12">
			<input type="tel" id="phone_no" name="phone_no"  class="form-control col-md-7 col-xs-12" value="<?php if(!empty($quot) && !empty($account)) echo $account->phone; ?>">
		</div>						
	</div>
</div>
<hr>	
 ...
 <div id="Customer-Details" class="container tab-pane "> 
      <div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
	     <div class="item form-group col-md-12 col-sm-12 col-xs-12">
							...
							<div class="col-md-6 col-sm-12 col-xs-12">								
								<input type="text" id="gstin" name="gstin" class="form-control col-md-7 col-xs-12 gstin" value="<?php if(!empty($quot) && !empty($account)) echo $account->gstin; ?>" onblur="fnValidateGSTIN(this)"> 
						</div>
						</div>  					
						<div class="item form-group col-md-12 col-sm-12 col-xs-12">
							<label class="col-md-3 col-sm-12 col-xs-12" for="address">Customer Address</label>
						...
						<div class="item form-group col-md-12 col-sm-12 col-xs-12">
							...
							<div class="col-md-6 col-sm-12 col-xs-12">
								<input type="email" id="email" name="email" class="form-control col-md-7 col-xs-12" placeholder="abcd@gmail.com" value="<?php if(!empty($quot) && !empty($account)) echo $account->email; ?>"> 
							</div>
						</div> 
	  ...
	  <div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
	        <div class="item form-group col-md-12 col-sm-12 col-xs-12">
							...
							<div class="col-md-6 col-sm-12 col-xs-12">
								<input type="tel" id="phone_no" name="phone_no"  class="form-control col-md-7 col-xs-12" value="<?php if(!empty($quot) && !empty($account)) echo $account->phone; ?>" readonly>
							</div>
						</div>						
						<div class="item form-group col-md-12 col-sm-12 col-xs-12">
							<label class="col-md-3 col-sm-12 col-xs-12" for="contact_person">Contact Person</label>
							<div class="col-md-6 col-sm-12 col-xs-12">
								<select class="contactPerson form-control" name="contact_id" data-id="contacts" data-key="id" data-fieldname="first_name" width="100%" id="contact_id" data-where="save_status = 1 AND account_id = <?php echo $quot->account_id ;?>">							
									<option value="">Select Option</option>
									<?php 
										if(!empty($quot)){
											$contact_id = $quot->contact_id;
											if($contact_id != 0){
											$contact = getNameById('contacts',$quot->contact_id,'id');
						...
						<div class="item form-group col-md-12 col-sm-12 col-xs-12">
							...
							<div class="col-md-6 col-sm-12 col-xs-12">
								<input type="text" id="contact_phone_no" name="contact_phone_no"  class="form-control col-md-7 col-xs-12" value="<?php if(!empty($quot) && !empty($contact)) echo $contact->phone; ?>" readonly>
							</div>
						</div>
						<div class="item form-group col-md-12 col-sm-12 col-xs-12">
							<label class="col-md-3 col-sm-12 col-xs-12" for="party_ref">Party Reference </label>
							<div class="col-md-6 col-sm-12 col-xs-12">
								<input type="text" id="party_ref" name="party_ref"  class="form-control col-md-7 col-xs-12" value="<?php if(!empty($quot)) echo $quot->party_ref; ?>">
							</div>
						</div>
	  </div>
 </div>
     ...
     <div class="item form-group col-md-6 col-sm-12 col-xs-12 vertical-border">	
		...
		<div class="col-md-6 col-sm-12 col-xs-12">
			<select class="form-control selectAjaxOption select2 select2-hidden-accessible material_type_id select2 add_material_cls materialTypeId requrid_class" required="required" id="material_type_id" name="material_type_id" id="" data-id="material_type" data-key="id" data-fieldname="name" data-where="created_by_cid=<?php echo $this->companyGroupId; ?> OR created_by_cid=0" tabindex="-1" aria-hidden="true" onchange="getMaterialName(event,this)" id="material_type">
				<option value="">Select Option</option>
				<?php
				if(!empty($quot) && $quot->material_type_id!=0){
					$material_type_id = getNameById('material_type',$quot->material_type_id,'id');
					echo '<option value="'.$quot->material_type_id.'" selected>'.$material_type_id->name.'</option>';
					}
				?>	
			</select>
						...
						if(!empty($products)){ 
								...
								?>
									<div class="well <?php if($i==1){ echo 'edit-row1 scend-tr mobile-view';}else{ echo 'scend-tr mobile-view';}?>"  id="chkWell_<?php echo $i; ?>" style="overflow:auto; ">
										...
										<div class="col-md-2 col-sm-6 col-xs-12 form-group">
										    <label>UOM</label>
											<input type="text" name="uom[]"  placeholder="uom" class="form-control col-md-7 col-xs-12" class="uom" value="<?php echo $product->uom ; ?>" readonly>
										</div>
										<div class="col-md-2 col-sm-6 col-xs-12 form-group">
										     <label>Price</label>
											<input type="text" name="price[]" id="inputSuccess2" placeholder="Price" class="form-control" value="<?php echo $product->price ;?>" onkeyup="keyupFunction(event,this)" onchange="keyupFunction(event,this)">
										...
										<div class="col-md-2 col-sm-6 col-xs-12 form-group">
										    <label>GST</label>
											<input type="text" name="gst[]" class="form-control col-md-7 col-xs-12 gst" value="<?php echo chop($product->gst,"%") ;?>" placeholder="gst%" readonly>
										</div>
										
											<input type="hidden" name="totals[]" class="form-control col-md-7 col-xs-12 total" value="<?php echo $product->total ;?>" readonly>
											
										
										
											<input  type="hidden" name="TotalWithGsts[]" class="form-control col-md-7 col-xs-12 totalWithGst" value="<?php echo $product->TotalWithGst ;?>" readonly>
				...
				<div class="col-md-12 col-sm-12 col-xs-12" style="clear:both; margin-top:22px;">
					<div class="col-md-4 col-sm-5 col-xs-12 text-right" style="float: right;">
						...
						<div class="col-md-7 col-sm-5 col-xs-6 text-right">
						  <input type="hidden"  name="total" required="required" class="form-control col-md-7 col-xs-12"  value="<?php if(!empty($quot)) echo $quot->total; ?>"> 
							<strong>Total:</strong>&nbsp;&nbsp;
							</div>
							<div class="col-md-5 col-sm-5 col-xs-6 text-left">
							      <span class="fa fa-rupee divSubTotal"><?php if(!empty($quot)){ echo $quot->total ; } else{ echo 0; }?></span>
							</div>
							 
						</div>
						<div class="col-md-12 col-sm-12 col-xs-12 text-right" style="font-size: 18px;color: #2C3A61; border-top: 1px solid #2C3A61;">
						<div class="col-md-7 col-sm-5 col-xs-6 text-right">
						  <input type="hidden"  name="grandTotal" required="required" class="form-control col-md-7 col-xs-12"  value="<?php if(!empty($quot)) echo $quot->grandTotal; ?>"> 
							<strong>Grand Total:</strong>&nbsp;
							</div>
							<div class="col-md-5 col-sm-5 col-xs-6 text-left">
							     <span class="fa fa-rupee divTotal"><?php if(!empty($quot)){ echo $quot->grandTotal ; } else{ echo 0; }?></span>
							</div>
...
<div class=" item form-group">													
		...
		<div class="col-md-6 col-sm-12 col-xs-12">
			<input type="tel" id="payment_terms" name="payment_terms"  class="form-control col-md-7 col-xs-12" value="<?php if(!empty($quot)) echo $quot->payment_terms; ?>">
		</div>						
	</div>
...
<div class="item form-group ">
		...
		<select class="terms_conditions selectAjaxOption select2 form-control" name="terms_conditions" data-id="termscond" data-key="content" data-fieldname="terms_tittle" width="100%" id="termscond" data-where="created_by_cid = <?php 	echo $this->companyGroupId; ?> AND save_status = 1">
			...
			echo '<option value="'.$quotation->content.'" selected>'.$quotation->tittle.'</option>';
							 }
								?>
									</select>
							</div>
...
<div class=" item form-group">													
		...
		<div class="col-md-6 col-sm-12 col-xs-12">
			<input type="tel" id="cash_discount" name="cash_discount"  class="form-control col-md-7 col-xs-12" value="<?php if(!empty($quot)) echo $quot->cash_discount; ?>">
		</div>						
	</div>
