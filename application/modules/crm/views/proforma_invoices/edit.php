...
//	}
	...
	if($pi_code_array[2] == ''){
		...
		if(@$so_code_array[3] !=''){
			$revisedPICode = $pi->pi_code.'_2'.'(Revised)';
			...
			if(@$pi_code_array[3] !=''){
				$orignalOrderCode = $pi_code_array[0].'_'.$pi_code_array[1].'_'.$pi_code_array[2];
				$currentRevisedPIChar = explode('(', @$pi_code_array[3], 1);
				$nextRevisedPIChar = chr(ord($currentRevisedPIChar[0]) + 1);
				$revisedPICode = $orignalOrderCode.'_'.$nextRevisedPIChar.'(Revised)';
			}else{
				$revisedPICode = $pi->pi_code.'_2'.'(Revised)';
...
*/ ?>
<form method="post" class="form-horizontal" action="<?php echo base_url(); ?>crm/saveProformaInvoice" enctype="multipart/form-data" id="leadForm" novalidate="novalidate">
			...
			<div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
				<div class=" item form-group form-group col-lg-12 col-md-12 col-sm-12 col-xs-12">
					...
					<div class="col-md-6 col-sm-12 col-xs-12">
						<input id="Purchase_code" class="form-control col-md-7 col-xs-12" name="pi_code" type="text" value="<?php if((!empty($pi))){ echo $pi->pi_code;} else { echo $piCode; } ?>" readonly>
					</div>
				</div>
				<div class=" item form-group form-group col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<label class="col-md-3 col-sm-12 col-xs-12" for="textarea">Order Date<span class="required">*</span></label>
					<div class="col-md-6 col-sm-12 col-xs-12">
						<input type="text" class="form-control has-feedback-left datePicker" name="order_date" id="order_date" value="<?php if(!empty($pi) && $pi->order_date!=''){ echo $pi->order_date; }else {echo date("d-m-Y");}?>" required="required" readonly>
				...
				<div class=" item form-group form-group col-lg-12 col-md-12 col-sm-12 col-xs-12">
					...
					<div class="col-md-6 col-sm-12 col-xs-12" style="top: 5px;">
						Part Load:<input type="radio" class="flat calcDiscount" name="load_type" id="genderM" value="part_load" <?php if(!empty($pi->load_type) && $pi->load_type == "part_load"){ echo "checked=checked"; } ?> required="required">
						...
						None:<input type="radio" class="flat calcDiscount noneradio" name="load_type" id="genderF" value="none" <?php if(!empty($pi->load_type) && $pi->load_type == "none"){ echo "checked=checked"; } ?>>
						<span id="radio_msg" style="font-size:12px;color:red;"></span>
					</div> 
	  ...
	  <div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
	        <div class="item form-group col-md-12 col-sm-12 col-xs-12">
						...
						<div class="item form-group col-md-12 col-sm-12 col-xs-12">
							...
							<div class="col-md-6 col-sm-12 col-xs-12">
								<input type="text" id="contact_name" name="contact_id" class="form-control col-md-7 col-xs-12" value="<?php if(!empty($pi) && !empty($account)) echo $account->contact_name; ?>" readonly>
								</div>
						</div>
						<div class="item form-group col-md-12 col-sm-12 col-xs-12">
			...
			<div class="">
				...
				<?php if(empty($pi)){ ?>
				    <div class="col-sm-12  col-md-12 label-box mobile-view2">
								   ...
								   <div class="col-md-1 col-sm-12 col-xs-12 form-group"><label>Box</label></div>
					...
					<div class="well scend-tr mobile-view"   id="chkIndex_1">
                       <div class="col-md-2 col-sm-12 col-xs-12 item form-group">
						...
						<div class="col-md-2 col-sm-12 col-xs-12 item form-group">
							...
							<select class="set_mat_name materialNameId form-control col-md-2 col-xs-12 selectAjaxOption select2 mat_name Add_mat_onthe_spot select2-width-imp select2-hidden-accessible" id="mat_name" required="required" name="product[]" onchange="getTax(event,this)" data-where="material_type_id != 213 OR created_by_cid=<?php echo $this->companyGroupId; ?> AND created_by_cid=0 AND status=1" data-id="material" data-key="id" data-fieldname="material_name" tabindex="-1" aria-hidden="true">
								<option value="">Select Option</option>
							</select>
						...
						<div class="col-md-1 col-sm-6 col-xs-12 form-group item">
						<label class="col-md-12" >Quantity</label>
							<input type="text" name="qty[]"  placeholder="Qty" class="form-control col-md-7 col-xs-12 quantity getStandardPack" onkeyup="keyupFunction(event,this)"  onchange="keyupFunction(event,this)" required="required">
						</div>
						<div class="col-md-1 col-sm-6 col-xs-12 form-group">
							<label>Box</label>
							<input type="text" name="box[]"  placeholder="box" class="BOXQTY form-control col-md-7 col-xs-12" value="" >
							<input type="hidden" name="standard_packing[]"  placeholder="box" class="standard_packingCls" value="" >
						</div>
						<div class="col-md-1 col-sm-6 col-xs-12 form-group">
						<label class="col-md-12">UOM</label>
							...
							<input type="hidden" class="uomid"  name="uom[]" readonly>
						</div>
						<div class="col-md-1 col-sm-6 col-xs-12 form-group">
								...
								?>
									<div class="well <?php if($i==1){ echo 'edit-row1 scend-tr mobile-view';}else{ echo 'scend-tr mobile-view';}?>"  id="chkWell_<?php echo $i; ?>" style="overflow:auto; ">
										...
										<div class="col-md-1 col-sm-12 col-xs-12 item form-group">
											...
											<textarea name="description[]" placeholder="Special Requirement" class="form-control col-md-7 col-xs-12" class="description"><?php echo $product->description ; ?></textarea>
										</div>
										...
										<div class="col-md-2 col-sm-12 col-xs-12 item form-group">
                                         ...
                                         <div class="col-md-1 col-sm-12 col-xs-12 item form-group">
											...
											if(!empty($mat_details->standard_packing)){
											...
											if(!empty($packing_value->packing_qty)){
											$packing_qty = $packing_value->packing_qty;
											} else {
											$packing_qty = 0;	
											}
											if(!empty($packing_value->packing_cbf)){
											$packing_cbf = $packing_value->packing_cbf;
											} else {
											$packing_cbf = 0;	
											}
											if(!empty($packing_value->packing_weight)){
											...
											if(!empty($packing_value->packing_qty) && !empty($packing_value->packing_cbf) && !empty($packing_value->packing_weight)){
											...
											@$total_cbf = $total_weight = $total_cbf_set = $total_weight_set = 0;	
											}
									    } else {
									    ...
									    echo '<img style="width: 50px; height: 50px;" src="'.base_url().'assets/modules/inventory/uploads/'.$attachments[0]['file_name'].'"><input type="hidden" name="pro_img[]" value="'.base_url().'assets/modules/inventory/uploads/'.$attachments[0]['file_name'].'">';
									  	} else {
										...
										<div class="col-md-1 col-sm-6 col-xs-12 form-group">
											...
											<input type="text" name="qty[]"  placeholder="Qty" class="form-control col-md-7 col-xs-12 quantity getStandardPack" value="<?php echo $product->quantity ;?>" onkeyup="keyupFunction(event,this)"  onchange="keyupFunction(event,this)">
										</div>
										<div class="col-md-1 col-sm-6 col-xs-12 form-group">
										    <label>Box</label>
											<input type="text" name="box[]"  placeholder="Box" class=" BOXQTY form-control col-md-7 col-xs-12" value="<?php echo $product->box ;?>" >
											<input type="hidden" name="standard_packing[]"  placeholder="box" class="standard_packingCls" value="<?php echo $product->standard_packing ;?>" >
										</div>
										<div class="col-md-1 col-sm-6 col-xs-12 form-group">
										    <label>UOM</label>
											<input type="text" name="uom1[]"  placeholder="uom" class="form-control col-md-7 col-xs-12 uom mat_uom" value="<?php
											// pre($product);
											$ww =  getNameById('uom', $product->uom,'id');
											echo 	$uom = !empty($ww)?$ww->ugc_code:'';
											?>" readonly>

											<input type="hidden" class="uomid" name="uom[]" readonly value="<?php echo $product->uom;  ?>">
										</div>
										<div class="col-md-1 col-sm-6 col-xs-12 form-group">
										     <label>Price</label>
				...
				<div class="col-md-12 col-sm-12 col-xs-12" style="clear:both; margin-top:22px;">
                     <div class="col-md-5 col-sm-5 col-xs-12 text-right" style="float: right;">
                        <div class="col-md-12 col-sm-12 col-xs-12 text-right" style="font-size: 12px;color: #2C3A61; ">
                           <label class="col-md-5 col-sm-12 col-xs-12" for="grand">Total Qty :</label>
                           <div class="col-md-6 col-sm-12 col-xs-12 text-left">
                              <input type="text" class="form-control col-md-7 col-xs-12" id="totalQtydata" name="totalQty" placeholder="Total Qty" value="<?php if(!empty($pi)){ echo $totalQty; } ?>"  readonly="">
                              
                           </div>
                        </div>
						<div class="col-md-12 col-sm-12 col-xs-12 text-right" style="font-size: 12px;color: #2C3A61; border-bottom: 1px solid #2C3A61;">
                           <label class="col-md-5 col-sm-12 col-xs-12" for="grand">Total Box :</label>
                           <div class="col-md-6 col-sm-12 col-xs-12 text-left">
                              <input type="text" class="form-control col-md-7 col-xs-12" id="totalBox" name="totalBox" placeholder="Total Box" value="<?php if(!empty($pi)){ echo $totalBox; } ?>"  readonly="">
		...
		<div class=" ">
			<div>
				...
				<div class="item form-group col-md-12 col-sm-12 col-xs-12">
						...
						<div class="col-md-6 col-sm-12 col-xs-12">
							<input id="discount_rate" class="form-control col-md-7 col-xs-12" name="discount_rate" type="text" value="<?php if(!empty($pi)){ echo $discount_rate; }else{ echo 0;} ?>" readonly="">
						</div>
					</div>
				<div class="item form-group col-md-12 col-sm-12 col-xs-12">
						<label class="col-md-3 col-sm-12 col-xs-12" for="discount_offered">CBF</label>
						<div class="col-md-6 col-sm-12 col-xs-12">
							<input id="pi_cbf" class="form-control col-md-7 col-xs-12" name="pi_cbf" type="text" value="<?php /*if(!empty($pi)){ echo $total_cbf_set; }*/ if(!empty($pi)) { echo $pi->pi_cbf; } else { echo '0'; }  ?>" readonly="">
						</div>
					</div>
					<div class="item form-group col-md-12 col-sm-12 col-xs-12">
						<label class="col-md-3 col-sm-12 col-xs-12" for="discount_offered">Weight</label>
						<div class="col-md-6 col-sm-12 col-xs-12">
							<input id="pi_weight" class="form-control col-md-7 col-xs-12" name="pi_weight" type="text" value="<?php /*if(!empty($pi)){ echo $total_weight_set; }*/ if(!empty($pi)) { echo $pi->pi_weight; } else { echo '0'; }  ?>" readonly="">
						</div>
					</div>
					...
					<div class="item form-group col-md-12 col-sm-12 col-xs-12">
						...
						<div class="col-md-6 col-sm-12 col-xs-12">
							<div class="checkbox pi_paymode_list">
								...
								if(!empty($pi_paymode)){ ?>
									<label><input onchange="showPermittedBy();" type="checkbox" class="flat" value="Cash" name="pi_paymode[]" <?php if(in_array('Cash', $pi_paymode)){ echo "checked";} ?>>Cash</label>
							<label><input onchange="showPermittedBy();" type="checkbox" class="flat" value="Debit Card" name="pi_paymode[]" <?php if(in_array('Debit Card', $pi_paymode)){ echo "checked";} ?>>Debit Card</label>
							<label><input onchange="showPermittedBy();" type="checkbox" class="flat" value="Credit Card" name="pi_paymode[]" <?php if(in_array('Credit Card', $pi_paymode)){ echo "checked";} ?>>Credit Card</label>
							<label><input onchange="showPermittedBy();" type="checkbox" class="flat" value="Cheque" name="pi_paymode[]" <?php if(in_array('Cheque', $pi_paymode)){ echo "checked";} ?>>Cheque</label>
							<label><input onchange="showPermittedBy();" type="checkbox" class="flat" value="Online Transfer" name="pi_paymode[]" <?php if(in_array('Online Transfer', $pi_paymode)){ echo "checked";} ?>>Online Transfer</label>
							<label><input onchange="showPermittedBy();" type="checkbox" class="flat" value="Credit" name="pi_paymode[]" <?php if(in_array('Credit', $pi_paymode)){ echo "checked";} ?>>Credit</label>
								<?php } else { ?>
									<label><input onchange="showPermittedBy();" type="checkbox" class="flat" value="Cash" name="pi_paymode[]">Cash</label>
							<label><input onchange="showPermittedBy();" type="checkbox" class="flat" value="Debit Card" name="pi_paymode[]">Debit Card</label>
							<label><input onchange="showPermittedBy();" type="checkbox" class="flat" value="Credit Card" name="pi_paymode[]">Credit Card</label>
							<label><input onchange="showPermittedBy();" type="checkbox" class="flat" value="Cheque" name="pi_paymode[]">Cheque</label>
							<label><input onchange="showPermittedBy();" type="checkbox" class="flat" value="Online Transfer" name="pi_paymode[]">Online Transfer</label>
							<label><input onchange="showPermittedBy();" type="checkbox" class="flat" value="Credit" name="pi_paymode[]">Credit</label>
								<?php } ?>
					...
					<div class="form-group col-lg-12 col-md-12 col-sm-12 col-xs-12" id="permitted_by" style="display:none;">
						...
						<div class="col-md-6 col-sm-12 col-xs-12">
							<input type="text" class="form-control discount_offered" name="pi_permitted"  value="<?php if(!empty($pi)) { echo $pi->pi_permitted; }  ?>" placeholder="Permitted By">
						</div>
					</div>  
					...
					<div class="form-group col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<label class="col-md-3 col-sm-12 col-xs-12" for="city">Extra Charges</label>
						<div class="col-md-6 col-sm-12 col-xs-12">
							<input onkeyup="ExtraCharges(event,this)" type="text" class="form-control discount_offered" name="extra_charges" id="extra_charges" value="<?php if(!empty($pi)) echo $pi->extra_charges; ?>" placeholder="Extra Charges in Rs">
						</div>
					</div>
				...
				<div class="item form-group col-md-12 col-sm-12 col-xs-12" id="sda_by" style="display:none;">
						...
						<div class="col-md-6 col-sm-12 col-xs-12">
						<select name="special_discount_authorized" id="sda_by_discount" class="form-control special_discount">
							<option value="">Select</option>
							<option value="Sh.Suresh Bansal Sir" <?php if(!empty($pi)){if($pi->special_discount_authorized == 'Sh.Suresh Bansal Sir') echo 'selected';}else { echo 'Suresh Bansal Sir'; }?> >Sh.Suresh Bansal Sir</option>
							<option value="Sh.Mukesh Bansal Sir" <?php if(!empty($pi)){if($pi->special_discount_authorized == 'Sh.Mukesh Bansal Sir') echo 'selected';}else { echo 'Mukesh Bansal Sir'; }?> >Sh.Mukesh Bansal Sir</option>
							<option value="Sh.Sohan Lal Bansal Sir" <?php if(!empty($pi)){if($pi->special_discount_authorized == 'Sh.Sohan Lal Bansal Sir') echo 'selected';}else { echo 'Sohan Lal Bansal Sir'; }?> >Sh.Sohan Lal Bansal Sir</option>
				...
				<div class="form-group col-md-12 col-sm-12 col-xs-12">
						...
						<div class="col-md-6 col-sm-12 col-xs-12" style="top: 5px;">
							ActualBasis:<input type="radio" class="flat" name="freight" id="genderM" value="FOR price" <?php if(!empty($pi) && $pi->freight == 'FOR price'){ echo 'checked';}elseif(empty($pi)){  echo 'checked'; }?>  />
				  ...
				  <div class="col-md-12 col-sm-12 col-xs-12 item form-group" id="freight1">
					...
					?>
					<div class="item form-group col-md-12 col-sm-12 col-xs-12">
						<label class="col-md-3 col-sm-12 col-xs-12">Comment</label>
						<div class="col-md-6 col-sm-12 col-xs-12">
						<textarea class="form-control" name="apply_comment"><?php if (!empty($pi->apply_comment)) {	echo $pi->apply_comment;	} ?></textarea>							
						</div>
					</div>
				   ...
				   <ul class="laserAddress space">
					  <li>
						 <div class="row">
							...
							<div class="col-md-7 text-right addColenAfter"><span class="blueColor text-nowrap divSubTotal"><?php if(!empty($subtotal)) { echo $subtotal; } ?></span></div>
						 </div>
					  </li>
					  <li>
						 <div class="row">
							<div class="col-md-5 pr-0 text-nowrap">Discount value <span  class="disRate"><?php if(!empty($pi)){ echo '( '. $discount_rate .' )'; }else{ echo '( 0 )';} ?></span>:-</div>
							<div class="col-md-7 text-right  addColenAfter">
							<span class="blueColor text-nowrap divDiscountValue">
					  ...
					  <li>
						 <div class="row">
							...
							<div class="col-md-7 text-right  addColenAfter"><span class="blueColor text-nowrap divTotal"><?php if(!empty($total)) { echo $total; } ?></span></div>
						 </div>
					  </li>
					  <li>
						 <div class="row">
							<div class="col-md-5 pr-0 text-nowrap">Tax:-</div>
							<div class="col-md-7 text-right  addColenAfter"><span class="blueColor text-nowrap divTax"><?php if(!empty($gst)) { echo $gst; } ?></span></div>
						 </div>
					  </li>
					  <li>
						 <div class="row">
							<div class="col-md-5 pr-0 text-nowrap">Freight Charge:-</div>
							<div class="col-md-7 text-right  addColenAfter"><span class="blueColor text-nowrap divFreightCharge"><?php if(!empty($freightCharges)) { echo $freightCharges; } ?></span></div>
						 </div>
					  </li>
					  <li>
						 <div class="row">
							<div class="col-md-5 pr-0 text-nowrap">GST on the Freight:-</div>
							<div class="col-md-7 text-right  addColenAfter"><span class="blueColor text-nowrap divGSTFreight"><?php if(!empty($gfc)) { echo $gfc; } ?></span></div>
						 </div>
					  </li>
					  <li>
						 <div class="row">
					  ...
					  <li>
						 <div class="row">
							...
							<div class="col-md-7 text-right  addColenAfter">
							<span class="blueColor text-nowrap divGrandTotal"><?php if(!empty($grand_total)) { echo $grand_total; } ?></span>
							</div>
						 </div>
					  </li>
					  <li>
						 <div class="row">
							...
							<div class="col-md-7 text-right  addColenAfter"><span class="blueColor text-nowrap divAdvancePaid"><?php if(!empty($advance_received)) { echo $advance_received; } ?></span></div>
						 </div>
					  </li>
					  <li>
						 <div class="row">
					  ...
					  <li>
						 <div class="row">
							...
							<div class="col-md-7 text-right  addColenAfter"><span class="blueColor text-nowrap divBalance"><?php if(!empty($remain_balance)) { echo $remain_balance; } ?></span></div>
...
<div class="bottom-bdr"></div>
	<div class="form-group">
		...
		<center>
			...
			<?php if((!empty($pi) && $pi->save_status !=1) || empty($pi)){
				echo '<input type="submit" class="btn edit-end-btn draftBtn" value="Save as draft" style="display:none;">';
			}

			 if(!empty($pi) && $pi->save_status ==1) { echo '<button id="send" type="submit" class="btn edit-end-btn">Revise PI</button>';  }else{ echo '<button id="send" type="submit" class="btn edit-end-btn SubmitBtn">Submit</button>'; } ?>
			</center>
		</div>
	</div>
            ...
            <div class="modal-content abc">
				...
				<form name="insert_party_data" name="ins"  id="insert_customer_data_id">
					<div class="modal-body">
						...
						<div class="item form-group col-md-12 col-sm-12 col-xs-12">
							<label class="control-label col-md-10 col-sm-10 col-xs-10" for="name">Buyer  Type<span class="required">*</span></label>
							<div class="col-md-10 col-sm-10 col-xs-8 form-group">
								<select class="form-control selectAjaxOption select2 variantoptId select2-hidden-accessible" required="required" name="type_of_customer" data-id="types_of_customer" data-key="id" data-fieldname="type_of_customer" tabindex="-1" aria-hidden="true" id="type_of_customer">
									<option value="">Select Option</option>
								</select>	
								<span class="spanLeft control-label"></span>
							</div>
