...
$currentRevisedSOChar = 'A';
$nextRevisedSOChar = chr(ord($currentRevisedSOChar) + 1);
$revisedSOCode = '';
if ($sale_order && $sale_order->save_status == 1) {


	if($sale_order->so_order == ''){
		echo " ";
	}

else{
	$so_code_array = explode('_', $sale_order->so_order, 4);
	...
	if($so_code_array[2] == ''){
		...
		$revisedSOCode = $sale_order->so_order.'_'.$currentRevisedSOChar.'(Revised)';
	}else if($so_code_array[2] != ''){
		#echo $po_code_array[2];
		$orignalOrderCode = $so_code_array[0].'_'.$so_code_array[1].'_'.$so_code_array[2];
		$currentRevisedSOChar = explode('(', $so_code_array[2], 1);
		$nextRevisedSOChar = chr(ord($currentRevisedSOChar[0]) + 1);
		$revisedSOCode = $orignalOrderCode.'_'.$nextRevisedSOChar.'(Revised)';
	}
...
<form method="post" class="form-horizontal" action="<?php echo base_url(); ?>crm/saveSaleOrder" enctype="multipart/form-data" id="leadForm" novalidate="novalidate">
		...
		<input type="hidden" name="revised_so_code" value="<?php if ($sale_order && !empty($sale_order)) { echo $revisedSOCode; } ?>" class="revised_po_code">
	...
	<div class="item form-group">
		...
		<div class="col-md-6 col-sm-12 col-xs-12">
			<input id="Purchase_code" class="form-control col-md-7 col-xs-12" name="so_order" type="text" value="<?php if( empty($sale_order) && (!empty($sale_order))){ echo $sale_order->so_order;} else { echo $soCode; } ?>" readonly>
		</div>
	</div>
		<div class=" item form-group">
		...
		<label class="col-md-3 col-sm-12 col-xs-12" for="company_branch">Company Unit<span class="required">*</span></label>
			<div class="col-md-6 col-sm-12 col-xs-12">
				<select class="form-control  select2 address" required="required" name="company_unit" id="company_unit">
					<option value="">Select Option</option>
						<?php
							if(!empty($sale_order)){
   ...
   <div id="Customer-Details" class="container tab-pane ">
      <div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
						...
						<div class="item form-group">
							...
							<div class="col-md-6 col-sm-12 col-xs-12">
								<input type="text" id="gstin" name="gstin" class="form-control col-md-7 col-xs-12" value="<?php if(!empty($account)) echo $account->gstin; ?>">
							</div>
						</div>
                       <div class="item form-group">
							<label class="col-md-3 col-sm-12 col-xs-12" for="email">Email</label>
							<div class="col-md-6 col-sm-12 col-xs-12">
								<input type="email" id="email" name="email" class="form-control col-md-7 col-xs-12" placeholder="abcd@gmail.com" value="<?php if(!empty($account)) echo $account->email; ?>">
							</div>
						</div>
      ...
      <div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
	      ...
	      <div class="item form-group">
							...
							<div class="col-md-6 col-sm-12 col-xs-12">
								<input type="tel" id="phone_no" name="phone_no"  class="form-control col-md-7 col-xs-12" value="<?php if(!empty($account)) echo $account->phone; ?>" readonly>
							</div>
						</div>
						<div class="item form-group">
...
<div id="Material-Details" class="container tab-pane active">
	...
	<div class=" form-group input_productre middle-box" style="position: relative; clear: both;">
		<?php
		if(empty($sale_order)){
			...
			?>
		         <div class="col-sm-12  col-md-12 label-box mobile-view2">
								<div class="col-md-2 col-sm-12 col-xs-12 form-group"><label>Material Type<span class="required">*</span></label></div>
								   ...
								   <div class="col-md-1 col-sm-12 col-xs-12 form-group"><label>Material Name<span class="required">*</span></label></div>
								   <div class="col-md-1 col-sm-12 col-xs-12 form-group"><label>Delivery Address</label></div>
                                  ...
                                  <?php }?>
								   <div class="col-md-1 col-sm-12 col-xs-12 form-group"><label>Alias</label></div>
				...
				<div class="well scend-tr mobile-view"   id="chkIndex_1">
					...
					<label  class="col-md-12">Material Type<span class="required">*</span></label>
						<select class="form-control selectAjaxOption select2 select2-hidden-accessible material_type_id select2 add_material_cls materialTypeId requrid_class" name="material_type_id[]" id="material_type_id" data-id="material_type" data-key="id" data-fieldname="name" data-where="created_by_cid=<?php echo $this->companyGroupId; ?> OR created_by_cid=0" tabindex="-1" aria-hidden="true" onchange="getMaterialName(event,this)" id="material_type">
						<option value="">Select Option</option>
						</select>
					</div>
					<?php  if ($Delivery[0]['crm_delivery_setting']==1) { ?>
						   <input type="hidden" name="crm_delivery_setting[]" id="crm_delivery_setting" value="<?=$Delivery[0]['crm_delivery_setting'];?>">

					 <div class="col-md-1 col-sm-12 col-xs-12 item form-group">
						<label  class="col-md-12">Material Name<span class="required">*</span></label>
						<!-- getTax_pl -->
						<select class="materialNameId  form-control selectAjaxOption select2 Add_mat_onthe_spot mat_status_on_change" required="required" name="product[]"  width="100%" onchange="getTax(event,this)" id="material">
						<option value="">Select Option</option>
						</select>
						...
						<textarea name="delivery_Add[]" placeholder="Delivery Adderss" class="form-control col-md-7 col-xs-12" class="delivery_Add"> </textarea>
						</div>
...
<!-- getTax_pl -->
					...
					<div class="col-md-1 col-sm-12 col-xs-12 item form-group">
						...
						<select name="alias[]"  id="aliasNameMat" class="form-control col-md-7 col-xs-12 select2" class="alias"></select>
					</div>
					...
					<label  class="col-md-12">QTY. </label>
						<input type="text" name="qty[]"  placeholder="Qty" class="form-control col-md-7 col-xs-12" class="quantity" onkeyup="keyupFunction(event,this)"  onchange="keyupFunction(event,this)">
					</div>
					<div class="col-md-1 col-sm-6 col-xs-12 form-group">
					...
					<label  class="col-md-12">Price </label>
						<input type="text" name="price[]"  placeholder="Price" class="form-control col-md-7 col-xs-12" onkeyup="keyupFunction(event,this)" onchange="keyupFunction(event,this)">
					</div>
					<div class="col-md-1 col-sm-6 col-xs-12 form-group">
					...
					<div class="col-sm-12 btn-row">
						<button class="btn edit-end-btn addProductButtonre" type="button" align="right">Add</button>
					</div>
					...
					if(!empty($products)){
						...
						?>
							...
							<select class="form-control selectAjaxOption select2 select2-hidden-accessible material_type_id select2 add_material_cls materialTypeId requrid_class" name="material_type_id[]" id="material_type_id" data-id="material_type" data-key="id" data-fieldname="name" data-where="created_by_cid=<?php echo $this->companyGroupId; ?> OR created_by_cid=0" tabindex="-1" aria-hidden="true" onchange="getMaterialName(event,this)" id="material_type">
								<option value="">Select Option</option>
								<?php
								if(!empty($product->material_type_id)){
								...
								?>
									 ...
									 <?php if ($Delivery[0]['crm_delivery_setting']==1) { ?>
                                       <input type="hidden" name="crm_delivery_setting[]" id="crm_delivery_setting" value="<?=$Delivery[0]['crm_delivery_setting'];?>">
                                        ...
                                        <div class="col-md-1 col-sm-12 col-xs-12 item form-group">
											...
											<select class="materialNameId  form-control selectAjaxOption select2 Add_mat_onthe_spot" name="product[]" data-id="material" data-key="id" data-fieldname="material_name" data-where="created_by_cid = <?php echo $this->companyGroupId; ?>" width="100%" onchange="getTax(event,this)" id="material">
												<option value="">Select Option</option>
												<?php
													if(!empty($product)){
								 ...
								 <div class="col-md-1 col-sm-12 col-xs-12 item form-group">
									<label>	Delivery Adderss	</label>
									<textarea name="delivery_Add[]" placeholder="Delivery Adderss" class="form-control col-md-7 col-xs-12" class="delivery_Add"><?php echo $product->delivery_Add ; ?></textarea>
							 	</div>
								...
								<div class="col-md-1 col-sm-12 col-xs-12 item form-group">
									<label  class="col-md-12">Alias </label>
									<select name="alias[]"  id="aliasNameMat" class="form-control col-md-7 col-xs-12 select2" class="alias">
										<option value="<?= @$product->alias ?>">
											<?= @$product->alias ?>
										</option>
									</select>
								</div>
								...
								<div class="col-md-1 col-sm-6 col-xs-12 form-group">
								<label>	Price	</label>
									<input type="text" name="price[]"  placeholder="Price" class="form-control col-md-7 col-xs-12 price" value="<?php echo $product->price ;?>" onkeyup="keyupFunction(event,this)" onchange="keyupFunction(event,this)">
								</div>
								<div class="col-md-1 col-sm-6 col-xs-12 form-group">
								<label>	GST	</label>
...
<?php $i++; }}}?>
<div class="col-sm-12 btn-row"><button class="btn edit-end-btn addProductButtonre" style="float: left;" type="button">Add</button></div>
...
<div class="col-md-12 col-sm-12 col-xs-12" style="clear:both; margin-top:22px;">
					<div class="col-md-4 col-sm-5 col-xs-12 text-right" style="float: right;">
						...
						<div class="col-md-7 col-sm-5 col-xs-6 text-right">
						  <input type="hidden"  name="total" required="required" id="total" class="form-control col-md-7 col-xs-12"  value="<?php if(!empty($sale_order)) echo $sale_order->total; ?>">
							<strong>Total:</strong>&nbsp;&nbsp;
							</div>
						...
						<div class="col-md-7 col-sm-5 col-xs-6 text-right">
						  <input type="hidden"  name="grandTotal" id="grandTotal" required="required" class="form-control col-md-7 col-xs-12"  value="<?php if(!empty($sale_order)) echo $sale_order->grandTotal; ?>">
...
<h3 class="Material-head">Dispatch Details<hr></h3>
	   <div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
		...
		<div class="col-md-12 col-sm-12 col-xs-12 item form-group">
			...
			<div class="col-md-6 col-sm-12 col-xs-12">
				<input type="number" class="form-control " name="agt" id="agt" value="<?php if(!empty($sale_order)) echo $sale_order->agt; ?>" onchange="keyupFunction(event,this)" onkeyup="keyupFunction(event,this)">
			</div>
		</div>
		<div class="col-md-12 col-sm-12 col-xs-12 item form-group">
	 ...
	 <div class="col-md-12 col-sm-12 col-xs-12 item form-group" id="freight1">
		<label class="col-md-3 col-sm-12 col-xs-12" for="purpose">Freight (Rs)</label>
	    	<div class="col-md-6 col-sm-12 col-xs-12">
	     <input type="text" id="freight" name="freightCharges" class="form-control col-md-7 col-xs-12 key-up-event" placeholder="Freight"  value="<?php  if (!empty($sale_order->freightCharges)) {	echo $sale_order->freightCharges;	} ?>"   min="0">
	   </div>
	 </div>
		<div class="col-md-12 col-sm-12 col-xs-12 item form-group">
		...
		<div class="col-md-12 col-sm-12 col-xs-12 item form-group">
			...
			<fieldset>
				<div class="control-group">
					<div class="controls">
					  <div class=" xdisplay_inputx form-group has-feedback">
						<input type="text" class="form-control has-feedback-left" id="payment_date" name="payment_date" id="single_cal3" aria-describedby="inputSuccess2Status3" value="<?php if(!empty($sale_order) && $sale_order->payment_date!=''){ echo $sale_order->payment_date; }else {echo date("d-m-Y");}?>">
						<span class="fa fa-calendar-o form-control-feedback left" aria-hidden="true"></span>
						<span id="inputSuccess2Status3" class="sr-only">(success)</span>
					  </div>
					</div>
		...
		<div class="col-md-12 col-sm-12 col-xs-12 item form-group">
			...
			<div class="col-md-6 col-sm-12 col-xs-12">
				<input type="text" class="form-control  discount_offered" name="advance_received" id="advance_received" value="<?php if(!empty($sale_order)) echo $sale_order->advance_received; ?>">
			</div>
		</div>
		...
		<div class="item form-group col-md-12 col-sm-12 col-xs-12">
			...
			<div class="col-md-6 col-sm-12 col-xs-12">
				<input type="text" class="form-control " name="label_printing_express" id="label_printing_express" value="<?php if(!empty($sale_order)) echo $sale_order->label_printing_express; ?>">
			</div>
		</div>
...
<div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
       <div class="item form-group col-md-12 col-sm-12 col-xs-12">
			...
			<div class="col-md-6 col-sm-12 col-xs-12">
				<input type="text" class="form-control " name="brand_label" id="brand_label" value="<?php if(!empty($sale_order)) echo $sale_order->brand_label; ?>">
			</div>
		</div>
		<div class="item form-group col-md-12 col-sm-12 col-xs-12">
		...
		<div class="item form-group col-md-12 col-sm-12 col-xs-12">
			...
			<div class="col-md-6 col-sm-12 col-xs-12">
				<input type="text" class="form-control " name="product_application" id="product_application" value="<?php if(!empty($sale_order)) echo $sale_order->product_application; ?>">
			</div>
		</div>
		<div class="item form-group col-md-12 col-sm-12 col-xs-12">
		...
		<div class="item form-group col-md-12 col-sm-12 col-xs-12 fields_wrap"></div>
		...
		<div class="form-group">
			<div class="col-md-6 col-md-offset-3">
			 ...
			 if(!empty($sale_order) && $sale_order->save_status ==1) { echo '<button id="send" type="submit" class="btn edit-end-btn">Revise SO</button>';  }else{ echo '<button id="send" type="submit" class="btn edit-end-btn">Submit</button>'; } ?>
			 <button type="button" class="btn btn-default " id="view_preview">Preview</button>
			</div>
		</div>
	</form>
				...
				<form name="insert_party_data" name="ins"  id="insert_Matrial_data_id">
					...
					<div class="item form-group col-md-12 col-sm-12 col-xs-12">
					  ...
					  <div class="col-md-10 col-sm-10 col-xs-8 form-group">
						<select class="selectAjaxOption select2 form-control" name="material_type_id" data-id="material_type" data-key="id" data-fieldname="name" width="100%" id="material_type_idr" data-where="created_by_cid = <?php 	echo $this->companyGroupId; ?> OR created_by_cid = 0">
							<option value="">Select Option</option>
...
<!--------------Quick add customer code original----------------------->
    ...
    <!--  privew  -->
 <div class="modal fade" id="previewData" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-large modal-dialog-centered new-modal" role="document" style="width: 90%; margin: 0px auto;">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Preview Sale Order </h5>
        <button type="button" class="close previewClose"   aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body"> </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary previewClose"  >Close</button> 
      </div>
    </div>
  </div>
</div>