...
<div class="col-md-12 col-xs-12 for-mobile">
      <div class="Filter Filter-btn2">
        <div class="col-md-4 datePick-right">
            ...
            <form action="<?php echo base_url(); ?>crm/Leads_Conversion_Across_Industries" method="get" id="date_range">
   ...
   <div class="stik">
      <div class="col-md-12 export_div">
         ...
         <div class="col-md-4 col-sm-12 datePick-right">
            <form action="<?php echo site_url(); ?>crm/Leads_Conversion_Across_Industries" method="get" id="export-form">
   ...
   <div class="x_content">
    <button type="button" class="btn btn-default buttons-copy buttons-html5 btn-sm print_clsss" id="bbtn">Print</button>
     ...
     <?php for ($i = 1; $i <= 6; $i++) { $months[] = date("Y-m-d", strtotime( date( 'Y-m-01' )." -$i months"));} ?>
      <table id="attendance123" class="table table-striped table-bordered account_index" cellspacing="0" width="100%">
         <thead>
            <tr>
               <th>Lead Industry</th>
               <th><?php echo date('F, Y',strtotime($months[5]));?> To <?php echo date('F, Y',strtotime($months[0]));?></th>
               <!-- <th>Lead Owner</th> -->
               <th>AVG OF LEAD CONVERSION TIME IN DAY(S)</th>
            </tr>
         </thead>
          <tbody>   
						...
						#	array_unique($invoices112);
                          $pageNos = array();
                           foreach($invoices11 as $invoices112){
                              ...
                              if(!empty($ert) && $ert->lead_industry !='' && $ert->lead_industry != 0) {  
                                $user = getNameById('add_industry',$ert->lead_industry,'id');
                                $user_name = !empty($user)?$user->industry_detl:''; 
                              }else { $user_name = 'N/A'; }           

                             if(!empty($ert) && $ert->lead_owner !='' && $ert->lead_owner != 0) {  
                                $ownername = getNameById('user_detail',$ert->lead_owner,'u_id');
                                $owner_name = !empty($ownername)?$ownername->name:''; 
                              }else { $owner_name = ''; }         
						?>
                  <?php
                   if(!in_array($ert->lead_industry, $pageNos)){ 
                    array_push($pageNos,$ert->lead_industry);
                  ?>
									  <tr>
                      <td> 
                        <?php 
                      ...
                      <td><?php #$yh = date("t", mktime(0,0,0, date("n") - 6));
                       $now = time(); // or your date as well
                       $your_date = strtotime($months[5]);
                       $datediff = $now - $your_date;
                       $rty = floor($datediff/(60*60*24));
                        echo bcdiv($total / $rty, 1, 2);  // 2.5
                      ?></td>
                    </tr>
                    <?php 
                      $output[] = array(
                        'Lead Industry' => !empty($user_name)?$user_name:'',
                        str_replace(',','', (date('F, Y',strtotime($months[5])) .'TO'.  date('F, Y',strtotime($months[0]))))
                         => !empty($total)?$total:'',
                        'AVG OF LEAD CONVERSION TIME IN DAY(S)' => bcdiv($total / $rty, 1, 2),
                      );   
                    ?>
            <?php } ?>        
								    <?php }} 
