...
<div class="col-md-12 col-xs-12 for-mobile">
      <div class="Filter Filter-btn2">
        <div class="col-md-4 datePick-right">
            ...
            <form action="<?php echo base_url(); ?>crm/lead_conversion_across_owners" method="get" id="date_range">
   ...
   <div class="stik">
      <div class="col-md-12 export_div">
         ...
         <div class="col-md-4 col-sm-12 datePick-right">
            <form action="<?php echo site_url(); ?>crm/lead_conversion_across_owners" method="get" id="export-form">
      ...
      <table id="attendance123" class="table table-striped table-bordered account_index" cellspacing="0" width="100%">
         <thead>
            <tr>
               ...
               <th>AVG OF SALE CYCLE DURATION IN DAY(S)</th>
            </tr>
         </thead>
          <tbody>   
              ...
              $pageNos = array();
                           foreach($invoices11 as $invoices112){
                              ...
                              if(!empty($ert) && $ert->lead_owner !='' && $ert->lead_owner != 0) {  
                                $user = getNameById('user_detail',$ert->lead_owner,'u_id');
                                $user_name = !empty($user)?$user->name:''; 
                              }else { $user_name = 'N/A'; }                    
						?>
                  <?php
                   if(!in_array($ert->lead_owner, $pageNos)){ 
                    array_push($pageNos,$ert->lead_owner);
                  ?>
									  <tr>
                      <td> 
                        <?php 
                     ...
                     <?php 
                      $output[] = array(
                        'Lead Owners' => !empty($user_name)?$user_name:'',
                        str_replace(',','', (date('F, Y',strtotime($months[5])) .'TO'.  date('F, Y',strtotime($months[0]))))
                         => !empty($total)?$total:'',
                        'AVG OF SALE CYCLE DURATION IN DAY(S)' => bcdiv($total / $rty, 1, 2),
                      );   
                    ?>
            <?php } ?>        
								    <?php }
