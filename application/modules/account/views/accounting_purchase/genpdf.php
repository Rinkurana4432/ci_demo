   ...
   $party_add = json_decode($party_ledger->mailing_address,true);
    ...
    $content .= '<table>
		...
		<table border="1" cellpadding="2">
			...
			$description = " <br />( Blank )";
			if( $dataPdf->description != '' ){
          $description = " <br /> ( {$dataPdf->description} )  ";
      }


				$content .= '<tr>
					<td  width="30px">'.$sno++.'</td>
					<td width="110px" >'.$sale_ledger->name. $description .'</td>
					<td width="70px">'.$dataPdf->hsn_code.'</td>
					<td width="70px"></td>
					<td width="75px"></td>
					<td width="70px"></td>
					<td width="86px">'.	money_format('%!i',$dataPdf->added_amt).'</td>
				</tr>';
				$k++;
					...
					if(!empty($dataPdf)){ //when charges added
						...
						if($dataPdf->CGST == 0 &&  $dataPdf->CGST == 0){
							$content .= '<tr>
								...
								<td>'.money_format('%!i',$dataPdf->added_amt).'</td>
							</tr>';
							...
							$content .='<tr>
								...
								$content .='<tr>
									...
									<td>'.money_format('%!i',round($dataPdf->TotalWithTax)).'</td>
								</tr>';
						}else{
		...
		if($j == $after_divide-1){
			...
			$Taxamount_total = 0;
				...
				if($dataPdf->CGST != 0 &&  $dataPdf->CGST != 0){
						...
						<td align="center">'.money_format('%!i',$dataPdf->added_amt).'</td>
						<td align="center">'.$cgst_sgst_tax.'%</td>
						<td align="center">'.money_format('%!i',$dataPdf->CGST).'</td>
						<td align="center">'.$cgst_sgst_tax.'%</td>
						<td align="center">'.money_format('%!i',$dataPdf->CGST).'</td>
						<td align="center">'.money_format('%!i',$dataPdf->totaltaxAMT).'</td></tr>';
						...
						<td align="center">'.money_format('%!i',$dataPdf->totaltaxAMT).'</td>
						<td align="center">'.money_format('%!i',$dataPdf->totaltaxAMT).'</td></tr>';
			...
			$content .='
				...
				if($dataPdf->CGST != 0 &&  $dataPdf->CGST != 0){
					...
					$content .='<tr>
								...
								<td align="center">'.money_format('%!i',$taxable_sum + $total_charges_amt).'</td>
								<td align="center"></td>
								<td align="center" colspan="1">'.money_format('%!i',$Taxamount_total  + $charges_freight_total).'</td>
								<td align="center"></td>
								<td align="center">'.money_format('%!i',$Taxamount_total + $charges_freight_total).'</td>
								<td align="center">'.money_format('%!i',$total_tax_sum + $freaight_tax_sum).'</td>
							</tr>';
