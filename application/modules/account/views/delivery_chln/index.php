<div class="x_content">
   <div class="col-md-12 col-xs-12 for-mobile">
      <div class="Filter Filter-btn2">
         <form class="form-search" method="get" action="<?= base_url() ?>account/delivery_chln">
            <div class="col-md-6">
               <div class="input-group">
                  <span class="input-group-addon">
                  <i class="ace-icon fa fa-check"></i>
                  </span>
                  <input type="text" class="form-control search-query" placeholder="Enter Challan Number" name="search" id="search" value="<?php if(!empty($_GET['search'])){echo $_GET['search'];}//echo $search_string;?>" data-ctrl="account/delivery_chln?tab=<?php if(isset($_GET['tab']))echo $_GET['tab'];?>">
                  ...
                  <a href="<?php echo base_url(); ?>account/delivery_chln?tab=<?php if(isset($_GET['tab'])){echo $_GET['tab'];}?>"><input type="button" name="submitSearchReset" class="btn btn-outline-secondary" value="Reset"></a>
                  </span>
               </div>
            </div>
         </form>
         ...
         <div id="demo" class="collapse">
            <div class="col-md-12 col-xs-12 datePick-left">
               ...
               <form action="<?php echo base_url(); ?>account/delivery_chln" method="get" id="date_range">
            ...
            <center>
               <div class="export_div">
                  <div class="btn-group"  role="group" aria-label="Basic example">
                     <?php if($can_add) {
                        //echo '<button type="button" class="btn btn-success addBtn add_challan_tabs" data-toggle="modal" id="add" data-id="add_challan"><i class="fa fa-plus-circle valignmiddle btnTitle-icon"></i>Add</button>';
						echo '<a href="'.base_url().'account/delivery_challan" class="btn btn-success addBtn"><i class="fa fa-plus-circle valignmiddle btnTitle-icon"></i>Add</a>';
                        } ?>
                  ...
                  <div class="col-md-3 datePick-right">
                     ...
                     <form action="<?php echo site_url(); ?>account/delivery_chln" method="get" id="export-form">
   ...
   <div class="" role="tabpanel" data-example-id="togglable-tabs">
      ...
      <div id="myTabContent" class="tab-content">
         <div role="tabpanel" class="tab-pane fade active in" id="tab_content_accepted_invoice" aria-labelledby="invoice_tab">
               ...
               <table id="" class="table table-striped table-bordered" data-id="account" style="width:100%" border="1" cellpadding="3">
                  <!--<table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">---->
                  <thead>
                     <tr>
                        <th scope="col">Id<span><a href="<?php echo base_url(); ?>account/delivery_chln?tab=challan&sort=asc" class="up"></a>
                           <a href="<?php echo base_url(); ?>account/delivery_chln?tab=challan&sort=desc" class="down"></a></span>
                        </th>
                        <th scope="col">Challan Number<span><a href="<?php echo base_url(); ?>account/delivery_chln?tab=challan&sort=asc" class="up"></a>
                           <a href="<?php echo base_url(); ?>account/delivery_chln?tab=challan&sort=desc" class="down"></a></span>
                        </th>
                        <th scope="col">Receiver Name<span><a href="<?php echo base_url(); ?>account/delivery_chln?tab=challan&sort=asc" class="up"></a>
                           <a href="<?php echo base_url(); ?>account/delivery_chln?tab=challan&sort=desc" class="down"></a></span>
                        </th>
                        <!--th>Sale Ledger</th-->
                  ...
                  <tbody>
                     <?php
                           ...
                           $baseImage = "";
                        		foreach($delivery_data as $challan_dtl){
                        				...
                        				if($can_edit) {
												...
												$action .=  '<a href="'.base_url().'account/editdelivery_chln/'.$challan_dtl["id"].'"  class="btn btn-edit  btn-xs" >Edit</a>';

                        						$action .=  '<a href="javascript:void(0)" id="'. $challan_dtl["id"] . '" data-id="challan_view_details" class="add_challan_tabs btn-view  btn  btn-xs"  id="' . $challan_dtl["id"] . '">View</a>';
                        						}
                        					if($can_delete) {
                        						$action = $action.'<a href="javascript:void(0)"  class="delete_listing btn    btn  btn-xs" "   data-href="'.base_url().'account/deleteChallan_details/'.$challan_dtl["id"].'" >Delete</a>';
                        ...
                        if($challan_dtl['save_status'] == 0){
                            $baseImage = base_url('assets/images/draft.png');
                            $draftImage = "<img src='{$baseImage}' class='img-circle' width='25%'>";
                        }
                        ...
                        echo "<tr>
                                <td data-label='id:'><span class='drftImgHide'>{$draftImage}</span>{$challan_dtl['id']} </td>
                                <td data-label='Challan Number:'>{$challan_dtl['challan_num']}</td>
                                <td data-label='Party Name:'>
                                    <a href='javascript:void(0)' id='{$challan_dtl['reciver_name']}' data-id='ledger_view' class='add_account_tabs'>
                                        {$party_name->name}
                                    </a>
                                </td>";
                        	    $material_id_datas = json_decode($challan_dtl['descr_of_goods'],true);
                				foreach($material_id_datas as $materialData){
                					if($materialData['material_id']!='' && $materialData['quantity'] !=''  ){
	                          ...
	                          echo "<td data-label='Material:'><a style='cursor: pointer;' class='add_challan_tabs' id='".$challan_dtl['id']."' data-toggle='modal' data-id='challan_mat_view_details'>".$materialName->material_name."</a></td>
                            	<td data-label='Challan Issue Date:'>".date("j F , Y", strtotime($challan_dtl['challan_date']))."</td>
         ...
         <div role="tabpanel" class="tab-pane fade" id="rejected_invoice_tab" aria-labelledby="auto_entery_tab">
            ...
            <table id="" class="table table-striped table-bordered" data-id="account" style="width:100%" border="1" cellpadding="3">
               ...
               <thead>
                  <tr>
                     <th scope="col">Id  <span><a href="<?php echo base_url(); ?>account/delivery_chln?tab=auto&sort=asc" class="up"></a>
                        <a href="<?php echo base_url(); ?>account/delivery_chln?tab=auto&sort=desc" class="down"></a></span>
                     </th>
                     <th scope="col">Challan Number<span><a href="<?php echo base_url(); ?>account/delivery_chln?tab=auto&sort=asc" class="up"></a>
                        <a href="<?php echo base_url(); ?>account/delivery_chln?tab=auto&sort=desc" class="down"></a></span>
                     </th>
                     <th scope="col">Party Name<span><a href="<?php echo base_url(); ?>account/delivery_chln?tab=auto&sort=asc" class="up"></a>
                        <a href="<?php echo base_url(); ?>account/delivery_chln?tab=auto&sort=desc" class="down"></a></span>
                     </th>
                     <!--th>Sale Ledger</th-->
                     <th scope="col">Material</th>
                     	...
                     	if(!empty($delivery_data_auto)){
                     		foreach($delivery_data_auto as $challan_dtl){
                     			if($challan_dtl['challan_date'] == ''){
                     				...
                     				$action = '';
                     				if($can_edit) {
                     						$action .=  '<a  href="javascript:void(0)" id="'. $challan_dtl["id"] . '" data-id="add_challan" class="btn btn-edit add_challan_tabs   btn-xs"  id="' . $challan_dtl["id"] . '">Edit</a>';

                     						$action .=  '<a href="javascript:void(0)" id="'. $challan_dtl["id"] . '" data-id="challan_view_details" class="add_challan_tabs btn-view  btn  btn-xs"  id="' . $challan_dtl["id"] . '">View</a>';
                     						}
                     					if($can_delete) {
                     						$action = $action.'<a href="javascript:void(0)"  class="delete_listing btn    btn  btn-xs" data-tooltip="Delete"   data-href="'.base_url().'account/deleteChallan_details/'.$challan_dtl["id"].'" >Delete</a>';
