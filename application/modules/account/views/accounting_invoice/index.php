...
<div class="col-md-12 col-xs-12 for-mobile">
      <div class="Filter Filter-btn2">
	       <form class="form-search" method="get" action="<?= base_url() ?>account/accounting_invoice">
			<div class="col-md-6">
			<div class="input-group">
					<span class="input-group-addon">
						<i class="ace-icon fa fa-check"></i>
					</span>
					<input type="text" class="form-control search-query" placeholder="Enter id,invoice number" name="search" id="search" value="<?php if(!empty($_GET['search'])){echo $_GET['search'];}//echo $search_string;?>" data-ctrl="account/accounting_invoice">
					...
					<span class="input-group-btn">
                        ...
                        <a href="<?php echo base_url(); ?>account/accounting_invoice"><input type="button" name="submitSearchReset" class="btn btn-outline-secondary" value="Reset"></a>
					</span>
				</div>
				</div>
			</form>	  
            	<form class="form-search" id="orderby" method="post" action="<?= base_url() ?>account/accounting_invoice">
			...
			<button style="margin-right: 0px !important;" type="button" class="btn btn-primary" data-toggle="collapse" data-target="#demo"><i class="fa fa-filter" aria-hidden="true"></i>Filter By<span class="caret"></span></button> 
								...
								<div class="control-group">
								  <div class="controls">
									<div class="input-prepend input-group">
									  ...
									  <input type="text" style="width: 200px" name="tabbingFilters" id="tabbingFilters" class="form-control" value=""  data-table="account/accounting_invoice"/>
									</div>
								  </div>
								</div>
							</fieldset>
							<form action="<?php echo base_url(); ?>account/accounting_invoice" method="get" id="date_range">	
			 ...
			 <?php
						...
						?>
				<form action="<?php echo site_url(); ?>account/accounting_invoice" method="get" id="select_from_brnch">
	...
	<div class="row hidde_cls">
					...
					<div class="export_div">
						...
						<div class="btn-group"  role="group" aria-label="Basic example">
						   <?php if($can_add) {
							...
							echo '<a href="'.base_url().'account/Create_accounting_invoice" class="btn btn-success addBtn"><i class="fa fa-plus-circle valignmiddle btnTitle-icon"></i>Add</a>';
						} ?>
			...
			<form action="<?php echo base_url(); ?>account/accounting_invoice" method="get" id="date_range">	
				...
				<form action="<?php echo site_url(); ?>account/accounting_invoice" method="get" id="export-form">
	...
	<div class="" role="tabpanel" data-example-id="togglable-tabs">
			<ul id="myTab" class="nav nav-tabs bar_tabs invoice-tab" role="tablist">
				<li role="presentation" class="active"><a href="#tab_content_accepted_invoice" id="invoice_tab" role="tab" data-toggle="tab" aria-expanded="true" onClick="invoice();">Accounting Invoice</a></li>
								...
								if($invoice['accept_reject'] != 1 ){	//'0','for accept','1','reject'	
										...
										if($can_edit) {
												if($invoice['created_date'] < $date){
													...
													$action .=  '<a href="'.base_url().'account/editaccountingInvoice_details/'.$invoice["id"].'"  class=" btn btn-edit  btn-xs" >Edit</a>';
												}
													$action .=  '<a href="javascript:void(0)" id="'. $invoice["id"] . '" data-id="accountinginvoice_view_details" class="add_invoice_details btn-view  btn  btn-xs"  id="' . $invoice["id"] . '">View</a>';
												}
 
												
											
											if($can_delete) { 
													$action = $action.'<a href="javascript:void(0)"  class="delete_listing btn  btn-delete  btn  btn-xs"   data-href="'.base_url().'account/deleteaccInvoice_details/'.$invoice["id"].'" >Delete</a>';
											}
