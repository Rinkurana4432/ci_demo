...
for($prnt = 1;$prnt<=$print_no_copies;$prnt++){
   ...
   $party_add = json_decode($party_ledger->mailing_address,true);
	...
	foreach ($sale_address_details as $comapny_sale_address_ldger) {
			...
			if ($comapny_sale_address_ldger['add_id'] == 1) {
    ...
    $content .= '<td ><h4 align="center">Tax Invoice</h4><span style="text-align:center; font-size:7px;">'.$no_copies.'</span></td>';
	if( $company_data->qr_code_img && $company_data->qr_code == 1 ){
	...
	$content .=  '</tr>
		...
		<table border="1" cellpadding="2">
			...
			<tr>
				...
				<th width="70px"><strong>HSN/SAC 1</strong></th>
...
/*
				...
				$content .='
					...
					<td width="60px">'.money_format('%!i',$data2[$k]['mat_rate_'.$k]).'</td>';
				...
				$content .=	'<td width="60px">'.($data2[$k]['discTotalAmt'.$k] + chargeAmountPerMat($dataPdf->charges_added,$basePrice - $data2[$k]['discTotalAmt'.$k] ,$dataPdf->descr_of_goods)).'</td>
					...
					<td width="70px">'.money_format('%!i',($data2[$k]['mat_qty_'.$k])*($data2[$k]['mat_rate_'.$k])). '</td>
				</tr>';
				...
				if($data2[$k]['particular_charges_name_'.$k] != ''){
                    ...
                    if( $charge_name->type_charges != 'minus' ){
                        $content .= '<tr>
                            ...
                            <td width="40px">1</td>
                            <td width="60px">'.money_format('%!i',$charges_added_without_tax).'</td>
                            <td width="60px">N/A</td>
                            <td width="70px">'.money_format('%!i',$charges_added_without_tax).'</td>
                            <td width="70px">'.money_format('%!i',$charges_added_without_tax).'</td>
                        </tr>';
...
// die;
				if($j == $after_divide-1){
					...
					if(!empty($data_charges_for_taxes)){ //when charges added
						...
						if($dataPdf->CGST == 0 &&  $dataPdf->CGST == 0){
							...
							$content .='<tr>
								...
								<td>'.money_format('%!i',($total_igst_val+$carges_taxx_igst)).'</td>
							</tr>';
							...
							if($invoice_amount_details[0]->cess_all_total != '' || $invoice_amount_details[0]->cess_all_total != 0){
								...
								<td>'.money_format('%!i',$invoice_amount_details[0]->cess_all_total).'</td>
							</tr>';
							}
							...
							<td>'.money_format('%!i',$invoice_amount_details[0]->total).'</td>
							</tr>';
						$content .='<tr>
							<td colspan="7" align="right"><strong>CGST </strong> </td>
							<td>'.money_format('%!i',$grand_total_charges_invoice_tax_half).'</td></tr>
							<tr><td colspan="7" align="right"><strong>SGST </strong> </td>
							<td>'.money_format('%!i',$grand_total_charges_invoice_tax_half).'</td>
							</tr>';
							if($invoice_amount_details[0]->cess_all_total != '' || $invoice_amount_details[0]->cess_all_total != 0){
								$content .='<tr>
								<td colspan="7" align="right"><strong>CESS </strong> </td>
					...
					if($dataPdf->CGST == 0 &&  $dataPdf->CGST == 0){
						$content .='<tr>
							<td colspan="7" align="right"><strong>IGST </strong> </td>
							<td>'.money_format('%!i',$dataPdf->IGST).'</td>
						</tr>';
					}else{
							$content .='<tr>
							<td colspan="7" align="right"><strong>CGST </strong> </td>
							<td>'.money_format('%!i',$dataPdf->CGST).'</td></tr>
							<tr><td colspan="7" align="right"><strong>SGST </strong> </td>
							<td>'.money_format('%!i',$dataPdf->SGST).'</td>
						</tr>';
					}
					...
					$content .='<tr>
							...
							<td>'.money_format('%!i',round($invoice_amount_details[0]->tds_tax)).'</td>
						</tr>';
					$roundoff =  round($dataPdf->total_amount) - $dataPdf->total_amount;
				 ...
				 $discountArrayValue = array_sum($discountArray);
					...
					$content .='<tr>
							...
							<td>'.money_format('%!i',(int)$grTotal).'</td>
						</tr>';
			}
		...
		if($j == $after_divide-1){
			...
			$content .= '<tr><td colspan="8"><br>Amount Chargeable(in Words)<br><b>'. $result . "Only".'</b></td></tr>';
			$content .= '<tr style="font-size:12px;background-color:#ddd;"><td colspan="6"><b>Outstanding Amount For '.$party_ledger->name.'</b></td><td colspan="2" style="text-align:center;font-size:12px;background-color:#ddd;"><b>'.$outStanding.'</b></td></tr>';
			if($dataPdf->CGST != 0 &&  $dataPdf->CGST != 0){
			$content .= '<tr>
							<td align="center" colspan="2">HSN/SAC </td>
							<td align="center"  colspan="1">Taxable value </td>
							...
							<td align="center" colspan="2">State Tax <br/>
								<table border = "1">
									<tr>
										<td>Rate</td>
										<td>Amount</td>
							...
							<td colspan="1" align="center">Total Tax Amount </td></tr>
						<tr>';
			}else{
				$content .= '<tr>
								<td colspan="2" align="center">HSN/SAC </td>
								<td colspan="2" align="center">Taxable <br> value </td>
								<td colspan="2" align="center">Integrated Tax <br/>
						<table border = "1">
							<tr>
								<td>Rate</td>
								<td>Amount</td>
						...
						<td colspan="2" align="center">Total Tax Amount</td>
					</tr>
			<tr>';
			}
			...
			$materialDetaileee =  json_decode($dataPdf->descr_of_goods);
				...
				if($dataPdf->CGST != 0 &&  $dataPdf->CGST != 0){
						$content .='<tr>
									<td colspan="2" align="center" style="height:20px;">'.$hsnsac_code.'</td>
									<td colspan="1" align="center" style="padding:0px; height:20px;">'.money_format('%!i',$taxable_val).'</td>
									<td align="center" style="padding:0px;height:20px;">'.$cgst_sgst_tax.'%</td>
									<td align="center" style="padding:0px;height:20px;">'.money_format('%!i',$cgst_sgst).'</td>
									<td align="center" style="padding:0px;height:20px;">'.$cgst_sgst_tax.'%</td>
									<td align="center" style="padding:0px;height:20px;" >'.money_format('%!i',$cgst_sgst).'</td>
									<td colspan="1" align="center" style="padding:0px;height:20px;">'.money_format('%!i',$total_tax_amount).' </td></tr>';
						...
						<td colspan="1" align="center">'.money_format('%!i',$taxable_val).'</td>
						<td align="center">'.$mat_tax.'%</td>
						<td align="center" >'.money_format('%!i',$total_ttax).'</td>
						<td align="center" colspan="3">'.money_format('%!i',$total_ttax).'</td></tr>';
	 ...
	 //die;
				...
				foreach($data_charges_data as $get_charge_data){
					...
					if($charge_name_val->type_charges == 'plus' && $get_charge_data['sgst_amt'] != '' &&  $get_charge_data['cgst_amt'] != ''){
						...
						$content .='<tr>
						<td colspan="1" align="center" style="padding:0px;height:20px;">'.$charge_name_Data->hsnsac.'</td>
						<td align="center" style="padding:0px;height:20px;">'.$get_charge_data['charges_added'].'</td>
						<td align="center" style="padding:0px;height:20px;">'.$get_charge_data['sgst_amt'].'%</td>
						<td align="center" style="padding:0px;height:20px;">'.money_format('%!i',$freight_taxable_amt/2).'</td>
						<td align="center" style="padding:0px;height:20px;">'.$get_charge_data['cgst_amt'].'%</td>
						<td align="center" style="padding:0px;height:20px;">'.money_format('%!i',$freight_taxable_amt/2).'</td>
						<td align="center" style="padding:0px;height:20px;">'.money_format('%!i',$freight_taxable_amt).'</td></tr>';
						...
						$content .='<tr>
							...
							<td align="center">'.money_format('%!i',$freight_taxable_amt).'</td>
							<td align="center">'.money_format('%!i',$freight_taxable_amt).'</td></tr>';
			...
			$content .='</tr>';
				if($dataPdf->CGST != 0 &&  $dataPdf->CGST != 0){
					...
					$content .='<tr>
								...
								<td align="center" colspan="1">'.money_format('%!i',$taxable_sum + $total_charges_amt).'</td>
								<td align="center" colspan="1"></td>
								<td align="center" colspan="1">'.money_format('%!i',$Taxamount_total  + $charges_freight_total).'</td>
								<td align="center" colspan="1"></td>
								<td align="center" colspan="1">'.money_format('%!i',$Taxamount_total + $charges_freight_total).'</td>
								<td align="center" colspan="1">'.money_format('%!i',$total_tax_sum + $freaight_tax_sum).'</td>
							</tr>';
					$content .='<tr><td colspan="8">Tax Amount (in Words)<br><b>'. $resulttax . "Only".'</b></td></tr>';
				}else{
					$content .='<tr>
								<td colspan="2" align="Right">Total</td>
								<td colspan="1" align="center">'.money_format('%!i',$taxable_sum + $total_charges_amt).'</td>
								<td colspan="1" align="center"></td>
								<td colspan="1" align="center">'.money_format('%!i',$total_tax_sum + $freaight_tax_sum).'</td>
								<td colspan="3" align="center">'.money_format('%!i',$total_tax_sum + $freaight_tax_sum).'</td>
							</tr>';
				$content .='<tr><td colspan="8">Tax Amount (in Words)<br><b>'. $resulttax . "Only".'</b></td></tr>';
		...
		$content .='<tr>
				...
				<td colspan="6">'.$company_data->company_pan.'</td>
			</tr>
			<tr >
                <td colspan="3"><h2><u> Declarations</u></h2>
					<p>'.$termandCondi.'</p>
				</td>
					<td class="align-bottom"  valign="bottom" colspan="3">
						...
						<td class="align-bottom"  valign="bottom" colspan="2" style="text-align:right;border:1px solid #ddd;">
