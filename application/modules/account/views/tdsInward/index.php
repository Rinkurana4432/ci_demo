...
<div class="col-md-12 col-xs-12 for-mobile">
      <div class="Filter Filter-btn2">
	       <form class="form-search" method="get" action="<?= base_url() ?>account/tdsInward">
			<div class="col-md-6">
			<div class="input-group">
					<span class="input-group-addon">
	 					<i class="ace-icon fa fa-check"></i>
					</span>
					<input type="text" class="form-control search-query" placeholder="Enter id,Purchase Bill Number" name="search" id="search" value="<?php if(!empty($_GET['search'])){echo $_GET['search'];}//echo $search_string;?>" data-ctrl="account/tdsInward">
					...
					<span class="input-group-btn">
                        ...
                        <a href="<?php echo base_url(); ?>account/tdsInward"><input type="button" name="submitSearchReset" class="btn btn-outline-secondary" value="Reset"></a>
					</span>
				</div> 
				</div>
			</form>	  
            	<form class="form-search" id="orderby" method="post" action="<?= base_url() ?>account/tdsInward">
			...
			<button style="margin-right: 0px !important;" type="button" class="btn btn-primary" data-toggle="collapse" data-target="#demo"><i class="fa fa-filter" aria-hidden="true"></i>Filter By<span class="caret"></span></button> 
								...
								<div class="control-group">
								  <div class="controls">
									<div class="input-prepend input-group">
									  ...
									  <input type="text" style="width: 200px" name="tabbingFilters" id="tabbingFilters" class="form-control" value="" data-table="account/tdsInward"/>
									</div>
								  </div>
								</div>
							</fieldset>
							<form action="<?php echo base_url(); ?>account/tdsInward" method="get" id="date_range">	
			 ...
			 <?php
						...
						?>
				<form action="<?php echo site_url(); ?>account/tdsInward" method="get" id="select_from_brnch">
	...
	<div class="row hidde_cls">
					...
					<div class="export_div">
						...
						<div class="btn-group"  role="group" aria-label="Basic example">
						   <?php if($can_add) {
							...
							echo '<a href="'.base_url().'account/Create_tdsInward" class="btn btn-success addBtn"><i class="fa fa-plus-circle valignmiddle btnTitle-icon"></i>Add</a>';
						} ?>
			...
			<form action="<?php echo base_url(); ?>account/tdsInward" method="get" id="date_range">	
				...
				<form action="<?php echo site_url(); ?>account/tdsInward" method="get" id="export-form">
	...
	<div class="" role="tabpanel" data-example-id="togglable-tabs">
			<ul id="myTab" class="nav nav-tabs bar_tabs invoice-tab" role="tablist">
				<li role="presentation" class="active"><a href="#tab_content_accepted_invoice" id="invoice_tab" role="tab" data-toggle="tab" aria-expanded="true" onClick="invoice();">Accounting Purchase Bill</a></li>
				
			</ul>
		<div id="myTabContent" class="tab-content">
		<div role="tabpanel" class="tab-pane fade active in" id="tab_content_accepted_invoice" aria-labelledby="invoice_tab">		
			
	    <form id="invoice_form"><input type="hidden" value="tdsinward" name="tab"></form> 
		<!------datatable-buttons----->
		<div id="print_div_content">
				...
				<thead>
					<tr>
						<th scope="col">Id<span><a href="<?php echo base_url(); ?>account/tdsInward?tab=tdsinward&sort=asc" class="up"></a>
							<a href="<?php echo base_url(); ?>account/tdsInward?tab=tdsinward&sort=desc" class="down"></a></span>
						</th>
						<th scope="col">Purchase Bill<span><a href="<?php echo base_url(); ?>account/tdsInward?tab=tdsinward&sort=asc" class="up"></a>
							<a href="<?php echo base_url(); ?>account/tdsInward?tab=tdsinward&sort=desc" class="down"></a></span>
							</th>
						<th scope="col">Supplier Name<span><a href="<?php echo base_url(); ?>account/tdsInward?tab=tdsinward&sort=asc" class="up"></a>
							<a href="<?php echo base_url(); ?>account/tdsInward?tab=tdsinward&sort=desc" class="down"></a></span>
						</th>
						<th scope="col">Sale Ledger<span><a href="<?php echo base_url(); ?>account/tdsInward?tab=tdsinward&sort=asc" class="up"></a>
						<a href="<?php echo base_url(); ?>account/tdsInward?tab=tdsinward&sort=desc" class="down"></a></span>
						</th>
						...
						<?php  
							...
							if(!empty($add_invoice_details)){
								foreach($add_invoice_details as $invoice){
									//pre($invoice);continue;
									if( $invoice['tds_details'] ){
										$tds_details = json_decode($invoice['tds_details'],true);
										$tdscharegeinallvelue = $subvalueinsubtds = "";
										foreach($tds_details as $key => $value){
											$subvalueinsubtds = $value['subvalueinsubtds'];
											$tdscharegeinallvelue = $value['tdscharegeinallvelue'];
								...
								if($invoice['accept_reject'] != 1 ){	//'0','for accept','1','reject'	
										...
										if($can_edit) {
												if($invoice['created_date'] < $date){
													...
													$action .=  '<a href="'.base_url().'account/editaccountingTDSdata_details/'.$invoice["id"].'"  class=" btn btn-edit  btn-xs" data-tooltip="Edit"><i class="fa fa-pencil"></i>  </a>';
												}
													$action .=  '<a href="javascript:void(0)" id="'. $invoice["id"] . '" data-id="tds_view_details" class="add_invoice_details btn-view  btn  btn-xs" data-tooltip="View" id="' . $invoice["id"] . '"><i class="fa fa-eye"></i> </a>';
												}
 
												
											
											if($can_delete) { 
													$action = $action.'<a href="javascript:void(0)"  class="delete_listing btn  btn-delete  btn  btn-xs" data-tooltip="Delete"   data-href="'.base_url().'account/deleteacctdsData_details/'.$invoice["id"].'" ><i class="fa fa-trash"></i></a>';
											}
					...
					$party_name = getNameById('ledger',$invoice['supplier_name'],'id');
				...
				echo "<tr>
					...
					<td data-label='Invoice Number:'>".$invoice['purchase_bill']."</td>
					...
					<td data-label='tax:'>".$invoice['TotalWithTax']."</td><td data-label='tax:'>".$invoice['totaltaxAMT']."</td>";
					
						
				$output[] = array(
							   'Invoice ID' => $invoice['id'],
							   'Invoice Number' => $invoice['purchase_bill'],
