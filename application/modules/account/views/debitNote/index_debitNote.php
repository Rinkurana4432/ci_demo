...
<div class="col-md-12 col-xs-12 for-mobile">
      <div class="Filter Filter-btn2">
		<form class="form-search" method="post" action="<?= base_url() ?>account/debitNoteview">
		<div class="col-md-6">
					<div class="input-group">
						<span class="input-group-addon">
							<i class="ace-icon fa fa-check"></i>
						</span>
						<input type="text" class="form-control search-query" placeholder="Enter Debit Note No" name="search" id="search" value="<?php if(!empty($_GET['search'])) echo $_GET['search'];?>" data-ctrl="account/debitNoteview">
						<span class="input-group-btn">
							<button type="submit" class="btn btn-purple btn-sm">
								<span class="ace-icon fa fa-search icon-on-right bigger-110"></span>
								Search
							</button>
			<a href="<?php echo base_url(); ?>account/debitNoteview">
			<input type="button" name="submitSearchReset" class="btn btn-outline-secondary" value="Reset"></a>
						</span>
					</div>
		</div>
		</form>	
		<form class="form-search" id="orderby" method="post" action="<?= base_url() ?>account/debitNoteview">
	...
	<?php if($can_add) {
		echo '<a href="'.base_url().'account/debitNote_create/"><button type="button" class="btn btn-success addBtn" data-toggle="modal" data-id="edit_hsn_ledger"><i class="fa fa-plus-circle valignmiddle btnTitle-icon"></i>Add</button></a>';
	} ?>
	</div>
	</div>
		   ...
		   if(!empty($creditNoteNo)){
			   ...
			   foreach($creditNoteNo as $val){
					...
					if($can_edit) {
						...
						$action .=  '<a href="'.base_url().'account/editdebitNote_details/'.$val["id"].'"  class=" btn btn-edit  btn-xs" >Edit</a>';
					}
					$action .=  '<a href="javascript:void(0)" id="'. $val["id"] . '" data-id="DRSaleReturn_view_details" class="add_CrSaleREturn_details  btn  btn-xs" data-tooltip="View" id="' . $val["id"] . '">View  </a>';
					if($can_delete) {	
						$action = $action.'<a href="javascript:void(0)" class="delete_listing btn  btn btn-info btn-xs" data-href="'.base_url().'account/deletedebitNote/'.$val["id"].'">Delete</a>';
					}
