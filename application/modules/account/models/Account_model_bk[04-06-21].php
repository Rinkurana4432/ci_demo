	...
	public function get_field_type_data($data, $table){
		...
		switch($table){
			...
			case 'user_dashboard':
				...
				break;
			case 'charges_lead':
				$all_fields = array('particular_charges','ledger_id','type_charges','tax_slab','created_by_cid','created_by_uid','hsnsac','amount_of_charges');
				break;
			 case 'inventory_flow':
			...
			case 'hsn_sac_master':
				...
				break;	
			case 'creditnote_tbl':
				$all_fields = array('crditNoteNo','date','customer_id','custmer_email','invoice_no','ledgerID','productDtl','amountDtl','created_by_cid','saleReturn_CN_ornot','created_by','sale_company_state_id','party_billing_state_id','IGST','CGST','SGST','creditAMt');
				break;	
			case 'debitnote_tbl':
				$all_fields = array('debitNoteNo','date','supplier_id','supplier_email','PurchaseBill_no','buyerID','productDtl','amountDtl','created_by_cid','PurchaseReturn_DN_ornot','created_by','sale_company_state_id','party_billing_state_id','IGST','CGST','SGST','debitAMt');
				break;
             case 'user':
...
//echo $result;die();		
	...
	public function get_data_day_book($table = '' , $where = array(), $limit, $start,$where2,$order) {$start = ($start-1) * $limit;
		...
		$dynamicdb->from($this->tablename);
		$dynamicdb->where($where);
		$dynamicdb->where_not_in('ledger_id','1,2,3,4,5,6');
	...
	function get_data_daybook($table = '' , $where = array(), $limit, $start,$where2,$order){
		...
		$dynamicdb->from($this->tablename);
		$dynamicdb->where($where);
		$dynamicdb->where("DATE_FORMAT(created_date,'%Y-%m-%d')",$current_date);
		$dynamicdb->where_not_in('ledger_id',1);
		$dynamicdb->where_not_in('ledger_id',2);
		$dynamicdb->where_not_in('ledger_id',3);
		$dynamicdb->where_not_in('ledger_id',4);
...
//echo $this->db->last_query();die();		
	...
	public function get_ledgers_details_using_group_byid($where) { 
		...
		// echo $dynamicdb->last_query();die();
		  ...
		  // $query = $dynamicdb->query("SELECT `ledger`.`id` as `ledgerid`, `transaction_dtl`.`ledger_id`,`transaction_dtl`.`debit_dtl`,`transaction_dtl`.`credit_dtl`,`transaction_dtl`.`type`,`transaction_dtl`.`type_id`,`transaction_dtl`.`add_date`, `ledger`.`id`,`ledger`.`name`,`ledger`.`account_group_id`,`ledger`.`parent_group_id`,`ledger`.`parent_group_id`,`ledger`.`opening_balance`,`ledger`.`compny_branch_id`,`openingbalc_cr_dr` FROM `ledger` LEFT JOIN `transaction_dtl` ON `ledger`.`id` = `transaction_dtl`.`ledger_id` WHERE ".$where." ORDER BY `ledger`.`account_group_id` ASC");
// return $query->result_array();
	...
	public function get_ledgers_details_for_balance_sheet($where) { 
		...
		// $dynamicdb->where('ledger.created_date <=', $second_date);
		// $dynamicdb->where('ledger.created_by_cid', $created_by_id );
		 // $dynamicdb->where('transaction_dtl.debit_dtl !=', 0);
		 // $dynamicdb->where('transaction_dtl.credit_dtl !=', 0);
		 $dynamicdb->where($where);
		//$dynamicdb->or_where('ledger.created_by_cid', 0 );
		$query = $dynamicdb->get();
		//echo $dynamicdb->last_query();die();
...
public function update_single_field($table, $data, $materialId) {
	...
	public function get_invoice_details_aging_report($table = '' ,$where = array(),$limit, $start,$where2,$order,$export_data,$over_Due_invoice) {
		...
		$dynamicdb->order_by('due_date' , "ASC");
		//$dynamicdb->limit($limit, $start);
