   <?php /*<div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
      <div class="item form-group">
         ...
         <input  class="customer_name_id" name="customer_name_id" type="hidden" value="<?php if(!empty($products_array)){ echo $products_array->account_id; }?>" >			
         <input  class="sale_order_no" name="sale_order_no" type="hidden" value="<?php if(!empty($products_array)){ echo $products_array->so_order; }?>" >			
   <div class="item form-group ">
         ...
         <div class="item form-group">
            <div class="col-md-12 input_holder middle-box">
				...
				if(!empty($products_array)){
				  $product_detail = json_decode($products_array->product);
				   $i =0;
				  foreach($product_detail as $sale_order_product){
				
				  $materialLoc_closing = getNameById('mat_locations',$sale_order_product->product,'material_name_id');
				...
				<div class="well <?php if($i==0){ echo 'edit-row1';}else{ echo 'scend-tr mobile-view';}?>" style="overflow:auto; border-top: 1px solid #c1c1c1 !important;" id="chkIndex_<?php echo $i; ?>">
				  <div class="col-md-2 col-sm-12 col-xs-12 form-group" style="display:none;">
					 ...
					 <!-- required="required" -->
					<select class="form-control selectAjaxOption select2  material_type_id select2"  name="material_type_id[]" data-id="material_type" data-key="id" data-fieldname="name" tabindex="-1" aria-hidden="false" data-where="created_by_cid=<?php echo $_SESSION['loggedInUser']->c_id; ?> OR created_by_cid=0" onchange="getMaterialName(event,this)" id="material_type">
					   <option value="">Select Option</option>
					   <?php 	 
						  $material_type_id = getNameById('material_type',$sale_order_product->material_type_id,'id');
						  echo '<option value="'.$material_type_id->id.'" selected>'.$material_type_id->name.'</option>';
						 ?>
					   </select>
				  </div> 
