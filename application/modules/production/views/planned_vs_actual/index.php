...
<?php }
   ...
   ?>
<div class="x_content">
   <form action="<?php echo site_url(); ?>production/planned_vs_actual" method="get" id="export-form">
   ...
   <div class="col-md-12 col-xs-12 for-mobile">
      <div class="Filter Filter-btn2">
         <form class="form-search " method="post" action="<?= base_url() ?>production/planned_vs_actual" >
            <div class="col-md-6">
               <div class="input-group">
                  <span class="input-group-addon">
                  <i class="ace-icon fa fa-check"></i>
                  </span>
                  <input type="text" class="form-control search-query" placeholder="Enter id" id="search" name="search" value="<?php if(!empty($_GET['search'])) echo $_GET['search'];?>" data-ctrl="production/planned_vs_actual">
                  <span class="input-group-btn">
                  <button type="submit" class="btn btn-purple btn-sm">
                  <span class="ace-icon fa fa-search icon-on-right bigger-110"></span>
                  Search
                  </button>
                  <a href="<?php echo base_url(); ?>production/planned_vs_actual"><input type="button" name="submitSearchReset" class="btn btn-outline-secondary" value="Reset"></a>
                  </span>
               </div>
            </div>
         </form>
         ...
         <div id="demo" class="collapse">
               ...
               <form action="<?php echo base_url(); ?>production/planned_vs_actual" method="get" id="date_range">	
      ...
      <div class="row hidde_cls export_div">
         <div class="col-md-12">
            ...
            <div class="btn-group"  role="group" aria-label="Basic example">
               ...
               <form action="<?php echo base_url(); ?>production/planned_vs_actual" method="get" >
      ...
      <table id="" class="table table-striped table-bordered" border="1" cellpadding="3" style="width:100%;" data-order='[[1,"desc"]]'>
         <thead>
            <tr>
               ...
               <th scope="col">Id
                  <span><a href="<?php echo base_url(); ?>production/planned_vs_actual?sort=asc" class="up"></a>
                  <a href="<?php echo base_url(); ?>production/planned_vs_actual?sort=desc" class="down"></a></span>
               </th>
               <th scope="col">Date
                  <span><a href="<?php echo base_url(); ?>production/planned_vs_actual?sort=asc" class="up"></a>
                  <a href="<?php echo base_url(); ?>production/planned_vs_actual?sort=desc" class="down"></a></span>
               ...
               <th scope="col">Planning Vs Actual %</th>
            ...
            <tr>
               <td><?php echo "<input name='checkbox[]' class='checkbox1 checkbox[]' type='checkbox'  value=" . $production_data['id'] . ">";
                  if ($production_data['favourite_sts'] == '1')
                  {
                  	...
                  	echo "<input type='hidden' value='production_data' id='favr' data-msg='Job card' data-path='production/planned_vs_actual' favour-sts='0' id-recd=" . $production_data['id'] . ">";
                  }
                  else
                  {
                  	echo "<input class='star-1' type='checkbox'  title='Mark Record' value=" . $production_data['id'] . ">";
                  	echo "<input type='hidden' value='production_data' id='favr' data-msg='job Card' data-path='production/planned_vs_actual' favour-sts='1' id-recd =" . $production_data['id'] . ">";
                  }
                  ?></td>
               <td data-label="Id:"><?php echo $draftImage . $production_data['id']; ?></td>
               <td data-label="Date:"><?php if ($production_data['date'] != '')
                  {
                  	echo date("j F , Y", strtotime($production_data['date']));
                  } ?> 
               </td>
               <td data-label="Shift:"><?php echo implode(", ", json_decode($shift, true)); ?> </td>
               <!-- prodData -->
               <?php
                  //pre($production_data);die;
                  if ($production_data['production_data'] != '')
                  {
                  	$production_planning = getNameById('production_planning', $production_data['planning_id'], 'id');
                  	$target_data= !empty($production_planning->planning_data) ? json_decode($production_planning->planning_data) : "";
                  	$actual_data = json_decode($production_data['production_data']);
                  	$percentageOfOutput = percentageOfOutput($actual_data , $target_data);
                  
                  } ?>
               <td data-label="Planning Vs Actual %:"><?php  echo $percentageOfOutput; ?>%</td>
               <td></td>
               <td data-label="Review:"><?php echo date("j F , Y", strtotime($production_data['created_date'])); ?></td>
               <td data-label="action:" class='hidde'>
                  <?php
                     if($can_view) 
                     echo '<button class="btn btn-view btn-xs productionTab" data-id="PlannedVsActualView" id="'.$production_data["id"].'" data-toggle="modal" data-tooltip="View"><i class="fa fa-eye"></i></button>';
                     ?>
               </td>
            </tr>
            <?php
               $output[] = array( 
               						   'Id' =>$production_data['id'],
               							'Date'	=>date("d-m-Y", strtotime($production_data['date'])),
               						   'Shift' => $shiftData->shift_name,
               						   'Planning Vs Actual' =>$percentageOfOutput);
               						   }
               			$planned_data  = $output;
               			export_csv_excel($planned_data); }?>
         </tbody>
      </table>
