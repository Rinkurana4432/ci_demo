...
<!-- 			
		...
		<?php } else{  
					...
					if(!empty($products)){ 
						...
						?>
							<div class="well"  id="chkIndex_<?php echo $i; ?>">
								<div class="item col-md-3 col-sm-12 col-xs-12 form-group">
									<?php  if($i == 1){  echo '<label>	Material Type	</label>'; }  ?>	
								  	<?php
			         	$material_type_name  = '';
		     	     	if(!empty($product) && $product->material_type_id !='' && $product->material_type_id!=0){
				     	$material_type_data = getNameById('material_type',$product->material_type_id,'id');
		    			$material_type_name = (!empty($material_type_data))? $material_type_data->name:'';
			     	}
		     		?>	
			<input type="hidden" name="material_type_id[]" data-id="material" width="100%" id="material" value="<?php if(!empty($sale_order)){ echo $sale_order->material_type_id;	} ?>"  >
