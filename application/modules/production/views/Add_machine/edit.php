...
<?php 			
	...
	$rId = $last_id + 1;
	$MachineCode = 'Wos'.rand(1, 1000000).'_'.$rId;					
?>
 <form method="post" class="form-horizontal" action="<?php echo base_url(); ?>production/saveAddMachine" enctype="multipart/form-data" id="purchaseIndentForm" novalidate="novalidate">
   ...
   <?php
		...
		<?php }else{ ?>	
		<input type="hidden" name="created_by" value="<?php if($Addmachine && !empty($Addmachine)){ echo $Addmachine->created_by;} ?>" >
		<?php } ?>
		<div class="col-md-6 col-sm-12 col-xs-12 vertical-border">
		<div class="item form-group">
			<label class="col-md-3 col-sm-3 col-xs-12">Machine group</label>
		...
		<div class="item form-group">
			<label class="col-md-3 col-sm-3 col-xs-12" for="company_branch">Company Branch<span class="required">*</span></label>
				<div class="col-md-6 col-sm-6 col-xs-12">
					<select class="form-control col-md-2 col-xs-12 selectAjaxOption select2" required="required" name="company_branch" data-id="company_address" data-key="compny_branch_id" data-fieldname="company_unit" tabindex="-1" aria-hidden="true" data-where="created_by_cid=<?php echo $_SESSION['loggedInUser']->c_id; ?>"  onChange="getDept(event,this)">
						...
						<?php
						if(!empty($Addmachine)){
							//$getUnitName = getNameById('company_address',$Addmachine->unit_name,'compny_branch_id');
			...
			<input type="hidden" name="val" value="<?php if(!empty($Addmachine)){  echo $Addmachine->company_branch; }?>" id="compny_branch_id">
			<div class="item form-group">
				...
				<div class="col-md-6 col-sm-6 col-xs-12">
					...
					*/?>
						<select class="form-control selectAjaxOption select2 select2-hidden-accessible select2 department" name="department"  tabindex="-1" aria-hidden="true">
								<option value="">Select Option</option>	
								<?php
									if(!empty($Addmachine)){
...
<div class="col-md-6 col-sm-12 col-xs-12 vertical-border">			
          ...
          <div class="item form-group">
			<label class="col-md-3 col-sm-3 col-xs-12" for="machine_name">Work Station Name<span class="required">*</span></label>
				<div class="col-md-6 col-sm-6 col-xs-12">
		  ...
		  <div class="item form-group">
             <label class="col-md-3 col-sm-3 col-xs-12" for="machine_code">Work Station Code</label>
                 <div class="col-md-6 col-sm-6 col-xs-12">
					<input id="machine_code" class="form-control col-md-7 col-xs-12" name="machine_code" placeholder="Work Station Code" required="required" type="text" value="<?php if(!empty($Addmachine)) echo $Addmachine->machine_code; else echo $MachineCode; ?>" >
                  </div>
          </div>
  ...
  <div class="tab-content">
    <div id="Machine-Parameters" class="tab-pane fade in active">	
	       <div class="item form-group blog-mdl">				
					<div class="col-md-12 col-sm-6 col-xs-12 form-group input_parameter middle-box">	
					<?php if(empty($Addmachine) || $Addmachine->machine_parameter == ''){  ?>
						<div class="well "  style=" border-top: 1px solid #c1c1c1 !important;" id="chkIndex_1">
							<div class="col-md-6 col-sm-12 col-xs-12 form-group">
								<label >Parameter</label>
								<input type="text" class="form-control item_name" name="machine_parameter[]" id="machine_parameters" placeholder="Enter machine Parameters" required="required">
							 </div>
							...
							<div class="col-sm-12 btn-row"><div class="input-group-append">
								<button class="btn edit-end-btn addButton" type="button" align="right">Add</button>
							</div></div>											
						</div>
						<?php } else {?>
	...
	<div id="Process" class="tab-pane fade in ">
	     <!-------------   Add more process and process type start-------------------->
			   ...
			   <div class="item form-group blog-mdl">
					<div class="col-md-12 col-sm-6 col-xs-12 form-group process_div middle-box">	
					<?php if(empty($Addmachine) || $Addmachine->process == ''){  ?>
							...
							<label>Process Type</label>
							 ...
							 <div class="col-md-6 col-sm-6 col-xs-12 form-group">
								<label style="border-right: 1px solid #c1c1c1 !important;">Process</label>								
								 <select style="border-right: 1px solid #c1c1c1 !important;" class="form-control selectAjaxOption select2 select2-hidden-accessible process_name_id" name="process_name[]" width="100%" tabindex="-1" aria-hidden="true" required="required" data-id="1add_process" id="process_name_id" data-key="id" data-fieldname="process_name" tabindex="-1" aria-hidden="true" data-where="created_by_cid=<?php echo $_SESSION['loggedInUser']->c_id; ?> <?php if(!empty($Addmachine) && $Addmachine->process_type !=''){  ?> AND process_type_id=<?php echo $mcProcess->process_type ; }?>">
									<option value="">Select Option</option>								
								</select>
							</div>
						...
						<div class="col-sm-12 btn-row"><div class="input-group-append">
								<button class="btn edit-end-btn processAddButton" type="button" align="right">Add</button></div>
							</div>	
						<?php } 
							 else {?>
							 <div class="col-sm-12 label-box">
						...
						<?php
						   ...
						   ?>					  
							<div class="well scend-tr" style="overflow:auto; border-top: 1px solid #c1c1c1 !important;" id="chkIndex_<?php echo $j; ?>"> 
								<div class="col-md-6 col-sm-12 col-xs-12 form-group">
			...
			<!-- code to start process type-->
		<?php /*	<div class="item form-group">
                <label class="control-label col-md-3 col-sm-3 col-xs-12" for="process_name">Process Type</label>
                    <div class="col-md-6 col-sm-6 col-xs-12">
					    <?php 
...
<div class="bottom-bdr"></div>				
 ...
 <div class="col-md-6 col-sm-12 col-xs-12 vertical-border">          
            <div class="item form-group">
                 <label class="col-md-3 col-sm-3 col-xs-12" for="model">Make And Model</label>
                    <div class="col-md-6 col-sm-6 col-xs-12">
            ...
            <div class="item form-group">
                <label class="col-md-3 col-sm-3 col-xs-12" for="year">Date Of Purchase</label>
                    <div class="col-md-6 col-sm-6 col-xs-12">
...
<div class="col-md-6 col-sm-12 col-xs-12 vertical-border">			
            <div class="item form-group">
                <label class="col-md-3 col-sm-3 col-xs-12" for="purpose">Placement Of Machine</label>
                    <div class="col-md-6 col-sm-6 col-xs-12">
