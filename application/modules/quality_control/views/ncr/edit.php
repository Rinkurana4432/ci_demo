	 ...
	 <div class="item form-group">
		<label class="col-md-3 col-sm-3 col-xs-12">Email Id</label>
			...
			for($i=0;$i<count($email1);$i++){?>
			   <input type="email" class="form-control" id="email_id" name="email[]" value="<?php echo $email1[$i]; ?>" > 
               <?php }}}}else{?>
               <input type="text" class="form-control " id="email_id" name="email[]" value="">    		<?php }?>
            </div>
    	...
    	<div class="item form-group fields_wrap_email"></div>
			  ...
			  <select id="custsaleorder" class="form-control selectAjaxOption select2"  data-id="saleorder" data-key="id" data-fieldname="name"name="saleorder_id" onchange="get_saleorder_products();" required>			<?php  if(!empty($edit->saleorder_id)){			$saleorder_nam = getNameById('sale_order',$edit->saleorder_id,'id');?> 			<option value="<?php echo $edit->saleorder_id ?>"><?php echo @$saleorder_nam->so_order;?></option><?php }?>			  </select>	
			  ...
			  <select id="custsaleorderproducts" class="form-control" name="product_id" required>			  	<?php  if(!empty($edit->product_id)){			$material_nam = getNameById('material',$edit->product_id,'id');?> 			<option value="<?php echo $edit->product_id ?>"><?php echo $material_nam->material_name;?></option><?php }?>			  </select>
					</div>
				</div>
