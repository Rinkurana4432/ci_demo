      ...
      <table id="example" class="table table-striped table-bordered" border="1" cellpadding="3" data-order='[[1,"desc"]]'>
         ...
         <tbody id="table_data">
            ...
            <tr>
               <td><?php echo $i; ?></td>
               <td>
                  <input type="text" name="report[insc<?= $i ?>][parameter]" style="width:80px" value="<?php echo $val->parameter;?>" readonly/>
               </td>
               <td>
                  <select class="instrument  form-control selectAjaxOption select2 commanSelect2" name="report[insc<?= $i ?>][instrument]"  width="100%" id="instrument" data-id="instrument" data-key="id" data-fieldname="name" data-where="created_by_cid='<?php echo $_SESSION['loggedInUser']->c_id; ?>'" tabindex="-1" aria-hidden="true" >
                     ...
                     <option value="<?php echo $val->instrument;?>"<?php if($value['id'] == $val->instrument){echo 'Selected';}?>><?php echo $value['name']; ?></option>
                     <?php }?> 
                  </select>
               </td>
               <td>
                  <select class="uom  form-control selectAjaxOption select2 commanSelect2" name="report[insc<?= $i ?>][uom1]"  width="100%" id="uom" data-id="uom" data-key="id" data-fieldname="uom_quantity" data-where="created_by_cid='0' || created_by_cid=<?php echo $this->companyGroupId; ?>" tabindex="-1" aria-hidden="true" >
                     ...
                     <option value="<?php echo $umoVal->id;?>" <?php if($umoVal->id==$val->uom1){echo 'Selected';}?>><?php echo $umoVal->uom_quantity;?></option>
                     <?php }?>
                  </select>
               </td>
               <td>
                  <input type="number" class="exp" name="report[insc<?= $i ?>][expectation]" value="<?php echo $val->expectation; ?>" style="width:70px" step="any" readonly onkeyup="calculate(this)" onclick="calculate(this)" />
               </td>
               <td>
                  <input type="number" class="min_dev" name="report[insc<?= $i ?>][deviation_min]" value="<?php echo $val->deviation_min; ?>" style="width:70px" min="-0.1"step="any" onkeyup="calculate(this)" onclick="calculate(this)" />
               </td>
               <td>
                  <input type="number" class="max_dev" name="report[insc<?=$i ?>][deviation_max]" value="<?php echo $val->deviation_max; ?>" style="width:70px" max="0.9" step="any" onkeyup="calculate(this)" onclick="calculate(this)" />
               </td>
               <td>
                  <input type="number" class="exp_min_dev" name="report[insc<?= $i ?>][exp_min_dev]" value="<?php echo $val->exp_min_dev; ?>" style="width:70px" readonly onkeyup="calculate(this)" onclick="calculate(this)"/>
               </td>
               <td>
                  <input type="number" class="exp_max_dev" name="report[insc<?= $i ?>][exp_max_dev]" value="<?php echo $val->exp_max_dev; ?>" style="width:70px" readonly onkeyup="calculate(this)" onclick="calculate(this)" />
               </td>
               <td><input type="number" style="width:70px" value="<?php echo $val->result; ?>" name="report[insc<?= $i ?>][result]"/></td>
               <td><input type="text" style="width:70px" value="<?php echo $val->remark; ?>" name="report[insc<?= $i ?>][remark]"/></td>
               <?php 
                  if( isset($val->coils) ){
               ...
               <td>
                  <input type="number" class="form-group obsInput obs<?= $val->id ?>" data-unique="<?= $val->id; ?>" name="report[insc<?= $i ?>][obs][obs_<?= $j; ?>]" 
                     value="<?php $obsSr = "obs_{$j}"; echo $obs->$obsSr??''; ?>" style="width:70px" />
               </td>
               <?php }
                  }  ?> 
               <td>
                  <input type="number" class="obsAvg" id="obsAvg<?= $val->id ?>" name="report[insc<?= $i ?>][avg]" value="<?= $val->avg??'' ?>" style="width:70px" readonly />
               </td>
               <td>
