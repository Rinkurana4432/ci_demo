...
class Chat extends ERP_Controller{
    ...
    public function _get_chats_messages($chat_id){
        ...
        if ($chats_messages->num_rows() > 0) {
			...
			//pre($chats_messages);die('dfdfdfdfdfdfdfdfdfdfd'); 
            ...
            foreach ($chats_messages->result() as $chats_messages) {
               ...
               // $li_class = ($this->session->userdata('user_id') == $chats_messages->user_id) ?'class="text-right"' : '';
			  // echo 'session===>>>'.$_SESSION['loggedInUser']->u_id;
			  // echo 'chatttt===>>>'.$chats_messages->user_id;
			   //echo 'ud===>>>'.$this->session->userdata('user_id');
                $li_class = ($this->session->userdata('user_id') == $chats_messages->user_id) ?'class="text-right"' : '';
                $image_class = ($this->session->userdata('user_id') == $chats_messages->user_id) ?'image-reply' : 'image';
				$a_class = ($this->session->userdata('user_id') == $chats_messages->user_id) ?'col-lg-6 col-md-6 col-md-offset-6 col-sm-12 col-xs-12':'';
                if ($chats_messages->is_image == '0') {
                    if ($chats_messages->is_doc == '1') {
						...
						'<li ' . $li_class .'>'
							...
							.'<span>&nbsp;'.$chats_messages->name.'</span> ' . $chats_messages->timestamp .'</span><br/>'
							.'<span class="message">'.$chats_messages->content.'</span>'
							.'</a></li>';
						
                       /* '<p><li ' . $li_class . '>'
                            . '<p class="message_content"><img src="'.base_url().'assets/modules/users/uploads/'.$avatar.'" alt="" class="img-responsive">
					...
					'<li ' . $li_class .'>'
							.'<a class="'.$a_class.'"><span class="' . $image_class .'"><img src="'.base_url().'assets/modules/users/uploads/'.$avatar.'" alt="" class="img-responsive"></span>'
							.'<span>&nbsp;'.$chats_messages->name.'</span> ' . $chats_messages->timestamp .'</span><br/>'
							// .'<p><a download="data_image" href="'.base_url().'assets/modules/chat/uploads/'.$chats_messages->content.'" title="ImageName"><img src="'.base_url().'assets/modules/chat/uploads/'.$chats_messages->content.'" class="image" width="100" height="100" /></a><i class="fa fa-download"></i></p>'
							// .'</a></li>';
							.'<a download="data_image" href="'.base_url().'assets/modules/chat/uploads/'.$chats_messages->content.'" title="ImageName"><img src="'.base_url().'assets/modules/chat/uploads/'.$chats_messages->content.'" class="image" width="100" height="100" /><i class="fa fa-download"></i></a>'
							.'</a></li>';
