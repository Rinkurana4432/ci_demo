...
class Ajaxrequest extends CI_Controller {
	...
	public function ajaxSelect2search(){
		...
		if(!empty($_SESSION['loggedInUser']) && $_SESSION['loggedInUser']->role == 3){
			...
			if( $this->input->get("table") == 'material' ){
				$fieldName = $this->input->get("fieldname");

				// $checkPCode = "CONCAT(material_code,' / ',{$fieldName}) as text";
				$checkPCode = "CONCAT(mat_sku,' / ',{$fieldName}) as text";
				$dynamicdb->select($this->input->get("field").' as id,'.$checkPCode);
			}else{
			...
			if(!empty($this->input->get("q"))){
				if( $this->input->get("table") == 'material' ){
					...
					$dynamicdb->or_like('material_code',$this->input->get("q"));*/
					//$dynamicdb->WHERE("({$table_field_name} LIKE '%{$this->input->get('q')}%' OR material_code LIKE '%{$this->input->get('q')}%' )");
					$dynamicdb->WHERE("({$table_field_name} LIKE '%{$this->input->get('q')}%' OR mat_sku LIKE '%{$this->input->get('q')}%' )");
				}else{
					$table_field_name = $this->input->get("fieldname");
					$dynamicdb->like(($table_field_name), $this->input->get("q"));
				}
			...
			if($this->input->get("fieldwhere")!=''){
				...
				/* company group end code */
				if( $this->input->get("table") == 'material' ){
					$dynamicdb->where('('.$whereCondition.') AND material_name != ""');
				}else{
