	 ...
	 var tags = $(".tm-input").tagsManager();
        $('input.typeahead').typeahead({
          source: function (query, process) {
            ...
            $.ajax({
              url: site_url + 'production/user_data/',
              type: 'POST',
              dataType: 'JSON',
              data: 'query=' + query,
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data != '') {
                    ...
                    init_select2();
                    var maxProcess = 10; //maximum input boxes allowed
...
$(function() {
    ...
    var dragClass = $(".dragg");
    var dragpreClass = $(".draggpreventive");
    ...
    if (dragpreClass.hasClass('draggpreventive')) {
        //if(($(this).hasClass('dragg')))
        //{
        console.log("afaf");
        draggableInitpreventive();
    }
...
function draggableInit() {
    ...
    $('.panel-body').bind('drop', function(event) {
        ...
        if (sourceId != targetId) {
            ...
            $.ajax({
                url: site_url + 'maintenance/changeProcessType/',
                dataType: 'json',
                type: 'POST',
                data: {
                    'processId': elementId,
...
function draggableInitpreventive() {
    var sourceId;
    $('[draggable=true]').bind('dragstart', function(event) {
        console.log('event===>>>', event);
        sourceId = $(this).parent().attr('id');
    ...
    $('.panel-body').bind('drop', function(event) {
        ...
        if (sourceId != targetId) {
            ...
            console.log("elementId->>", elementId);
            $.ajax({
                url: site_url + 'maintenance/changeProcessTypepre/',
                dataType: 'json',
                type: 'POST',
                data: {
                    'processId': elementId,
...
/*******************************************************Other modules*******************************************/
$(document).ready(function(e) {
    $(document).on("click", ".maintenanceTab", function(ev) {
        ev.preventDefault();
        var id = $(this).attr('id');
        ...
        switch (tab) {
            ...
            case 'editbreakdown':
                ...
                break;
            case 'addSimilarbreakdown':
                url = 'maintenance/addsimiller';
                break;

            case 'viewbreakdown':
                url = 'maintenance/viewmaintenance';
                break;
            ...
            case 'acknowledgedate':
                url = 'maintenance/aknowledge/';
                break;
            case 'purchase':
                url = 'maintenance/purchase/';
                break;
            case 'completedbreakdown':
                url = 'maintenance/complete/';
                break;

            case 'viewbreakdowncom':
                url = 'maintenance/completeview';
                break;

            case 'viewpurchase':
                url = 'maintenance/viewpurchase';
                break;

            case 'setpreventive':
                url = 'maintenance/setpreventive';
                break;

            case 'viewpreventive':
                url = 'maintenance/viewpreventive';
                break;
			case 'viewpreventivedtl':
                url = 'maintenance/viewpreventivedtl';
                break;		

            case 'editpreventive':
                url = 'maintenance/editpreventive';
                break;

            case 'editpipline':
                url = 'maintenance/editpipline';
                break;

            case 'prepipeline':
                url = 'maintenance/prepipeline';
                break;

            case 'prepipelinesedule':
                url = 'maintenance/prepipelinesedule';
                break;

            case 'prepipelinedone':
                url = 'maintenance/pipelinedone';
                break;
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data != '') {
                    ...
                    $('#maintenance_modal .modal-body-content').html(data);
                    ...
                    if (tab == 'purchase') {
                       ...
                       // piaddCharges();
                        ...
                        addMaterialDetail2();
                        addMaterialDetail3();
                        subaddMaterialDetail2();
                        subaddMaterialDetail3();
                    }
...
$(document).ready(function() {
    ...
    $("#del_all").on('click', function(e) {
        ...
        if (confirm('Are You Sure!') == true) {
            ...
            $.ajax({
                url: site_url + 'maintenance/deleteall/',
                type: 'post',
                data: {
                    tablename: tablename,
...
$(".star-1").on('change', function(e) {
    ...
    $.ajax({
        url: site_url + 'maintenance/markfavourite/',
        type: 'post',
        data: {
            tablename: tablename,
            checkValues: checkValues,
...
/* function addMaterialDetail() {
    ...
    $(add_mat).click(function(e) {
        ...
        // measurmentArray = measurmentArray+'<option value="'+value+'">'+value+'</option>';
        // });
		 var y = $(this).closest("input_fields_wrap").find(".well").length;
		var input_mat = $(this).closest(".input_holder");
        if (y < input) {
            y++;
            $(input_mat).append('<div class="well scend-tr mobile-view" id="chkIndex_' + y + '" ><div class="col-md-2 col-sm-12 col-xs-12 form-group"><label>Material Type</label><select class="form-control selectAjaxOption select2 select2-hidden-accessible material_type_id select2" required="required" name="material_type_id[]" data-id="material_type" data-key="id" data-fieldname="name" tabindex="-1" aria-hidden="true" data-where="created_by_cid=' + logged_user + ' OR created_by_cid=0" onchange="getMaterialName(event,this)" id="material_type"><option value="">Select Option</option></select></div><div class="col-md-2 col-sm-12 col-xs-12 form-group"><label>Material Name</label><select  class="materialNameId form-control col-md-2 col-xs-12 selectAjaxOption select2" id="mat_name" required="required" name="material_name[]" onchange="getUom(event,this);"></select></div><div class="col-md-2 col-sm-12 col-xs-12 form-group"><label>Quantity</label><input id="qty" class="form-control col-md-7 col-xs-12 qty" name="quantity[]" placeholder="Qty" required="required" type="text" onkeyup="keyupFunction(event,this)" onchange="keyupFunction(event,this)"></div><div class="col-md-2 col-sm-12 col-xs-12 form-group"><label>UOM</label><input type="text" id="uom" name="uom_value[]" class="form-control col-md-7 col-xs-12  uom" style="width:100%;" placeholder="uom." value="" readonly></div><div class="col-md-2 col-sm-12 col-xs-12 form-group"><label>Price</label><input type="text" name="price[]" class="form-control col-md-7 col-xs-12 " placeholder="Price" value="" onkeyup="keyupFunction(event,this)" onchange="keyupFunction(event,this)"></div><div class="col-md-2 col-sm-12 col-xs-12 form-group"><label>Total</label><input style="border-right: 1px solid #c1c1c1 !important;" type="text" name="total[]" class="form-control col-md-7 col-xs-12  total" placeholder="Total Amount" value="" readonly></div><button class="btn btn-danger remove_input" type="button"><i class="fa fa-minus"></i></button></div>');
...
function addMaterialDetail2(well) {
    var input = 10;
    var input_mat = $(".input_holder");
    var add_mat = $(".addmaterial");
    var logged_user = $('#loggedUser').val();
    ...
    $(add_mat).click(function(e) {
        ...
        if (y < input) {
            y++;
            $(input_mat).append('<div class="well scend-tr mobile-view" id="chkIndex_' + y + '" ><div class="col-md-11 col-sm-12 col-xs-12 form-group"><label>Check List</label><input style="border-right: 1px solid #c1c1c1 !important;" type="text" name="check_list_data[]" class="form-control col-md-7 col-xs-12  check_list" placeholder="Check List" value=""></div><div class="col-md-1 form-group" style="padding: 18.7px;border-right: 1px solid #c1c1c1 !important;border-bottom: 1px solid #c1c1c1 !important;"></div><button class="btn btn-danger remove_input" type="button"><i class="fa fa-minus"></i></button></div>');
...
function subaddMaterialDetail2() {
    var input = 10;
    var input_mat = $(".input_holdersub");
    var add_mat = $(".addmaterialsub");
    var logged_user = $('#loggedUser').val();
	var y = $('.input_fields_wrapsub .wellsub').length;
    //var y = 1; 
    $(add_mat).click(function(e) {
        // e.preventDefault();
        // var measurmentArray = '';
        ...
        // });
		// var y = $('.input_fields_wrapsub .wellsub').length;
		 var y = $(this).closest(".input_fields_wrapsub").find(".wellsub").length;
	var input_mat = $(this).closest(".input_holdersub");
        if (y < input) {
            y++;
            $(input_mat).append('<div class="well scend-tr mobile-view" id="chkIndex_' + y + '" ><div class="col-md-11 col-sm-12 col-xs-12 form-group"><label>Check List</label><input style="border-right: 1px solid #c1c1c1 !important;" type="text" name="check_list_data[]" class="form-control col-md-7 col-xs-12  check_list" placeholder="Check List" value=""></div><div class="col-md-1 form-group" style="padding: 18.7px;border-right: 1px solid #c1c1c1 !important;border-bottom: 1px solid #c1c1c1 !important;"></div><button class="btn btn-danger remove_input" type="button"><i class="fa fa-minus"></i></button></div>');
...
/*job card add more material field */
function addMaterialDetail3() {
    var input = 10;
    var input_mat = $(".input_holder2");
    var add_mat = $(".addmaterial2");
    var logged_user = $('#loggedUser').val();
    //var y = 1; 
    var y = $('.input_fields_wrap2 .well2').length;
    $(add_mat).click(function(e) {
        // e.preventDefault();
        // var measurmentArray = '';
        // $.each( measurementUnits, function( key, value ) {
        ...
        if (y < input) {
            y++;
            $(input_mat).append('<div class="well scend-tr mobile-view" id="chkIndex_' + y + '" ><div class="col-md-4 col-sm-12 col-xs-12 form-group" style="padding: 0px;" ><label style="border-right: 1px solid #c1c1c1 !important;">Material Type</label><select class="form-control selectAjaxOption select2 select2-hidden-accessible material_type_id select2" name="material_type[]" data-id="material_type" data-key="id" data-fieldname="name" tabindex="-1" aria-hidden="true" data-where="created_by_cid=' + logged_user + ' OR created_by_cid=0"  onchange="getMaterialName(event,this)" id="material_type"><option value="">Select Option</option></select></div><div class="col-md-4 col-sm-12 col-xs-12 form-group" style="padding: 0px;" ><label style="border-right: 1px solid #c1c1c1 !important;">Material Name</label><select  class="materialNameId form-control col-md-2 col-xs-12 selectAjaxOption select2 Add_mat_onthe_spot" id="mat_name"  name="material_name[]" onchange="getUom_input(event,this);"><option value="">Select Option</option></select></div><div class="col-md-3 col-sm-12 col-xs-12 form-group"><label>Quantity</label><input style="border-right: 1px solid #c1c1c1 !important;" type="text" name="quantity[]" class="form-control col-md-7 col-xs-12  quantity" placeholder="Quantity" value=""></div><div class="col-md-1 form-group" style="padding: 18.7px;border-right: 1px solid #c1c1c1 !important;border-bottom: 1px solid #c1c1c1 !important;"></div><button class="btn btn-danger remove_input" type="button"><i class="fa fa-minus"></i></button></div>');
...
function subaddMaterialDetail3() {
    var input = 10;
    var input_mat = $(".input_holder2sub");
    var add_mat = $(".addmaterial2sub");
    var logged_user = $('#loggedUser').val();
    //var y = 1; 
    var y = $('.input_fields_wrap2sub .well2sub').length;
    $(add_mat).click(function(e) {
        // e.preventDefault();
        // var measurmentArray = '';
        // $.each( measurementUnits, function( key, value ) {
...
function machineProcess() {
    ...
    $(add_machine).click(function(e) {
        ...
        if (x < machine) {
            ...
            $(machine_detail).append('<div class=" well2 scend-tr" id="chckIndex_' + well + '" data-id="frst_div_' + well + '"><div class="col-md-6 col-sm-12 col-xs-12 vertical-border"><div class="item form-group"><label class="col-md-6 col-sm-12 col-xs-12" for="product detail">Frequency<span class="required">*</span></label><div class="col-md-6 col-sm-12 col-xs-12"><select class="form-control" name="frequency_data[]" required="required"><option value="">Select Option</option><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option><option value="hafe yearly">Hafe Yearly</option><option value="yearly">Yearly</option></select></div></div></div><div class="col-md-6 col-sm-12 col-xs-12 vertical-border"><div class="item form-group"><label class="col-md-3 col-sm-2 col-xs-4" for="description">Start date</label><div class="col-md-7 col-sm-10 col-xs-8"><input required="required" type="datetime-local" class="form-control has-feedback-left datePicker" name="start_date[]" id="start_date" value=""><span class="fa fa-calendar-o form-control-feedback left" aria-hidden="true"></span><span id="inputSuccess2Status3" class="sr-only">(success)</span></div></div></div><div class="item form-group "><div class="col-md-12 col-sm-12 col-xs-12 input_fields_wrapsub" data-id="' + well + '"><div class="item form-group"><div class="col-md-8 input_holdersub middle-box" data-id="' + well + '" style="margin: 0px auto; float: unset;"><div class=" well  wellsub" id="chkIndex_1" data-id="' + well + '" style=" overflow:auto; border-top:1px solid #c1c1c1;"><div class="col-md-11 col-sm-12 col-xs-12 form-group" style="padding:0px;"><label style="border-right: 1px solid #c1c1c1 !important;">Check List</label><input style="border-right: 1px solid #c1c1c1 !important;" type="text" required="required" name="check_list_data[]" class="form-control col-md-7 col-xs-12 check_list" placeholder="Check List" value=""></div><div class="col-md-1 col-sm-12 col-xs-12 form-group" style="padding: 0px;"><label style="border-right: 1px solid #c1c1c1 !important;">Action</label><div class="input-group-append1"><button class="btn edit-end-btn addmaterialsub " data-id="' + well + '" type="button"><i class="fa fa-plus"></i></button></div></div></div></div></div></div></div><div class="item form-group "><div class="col-md-12 col-sm-12 col-xs-12 input_fields_wrap2sub"><div class="item form-group2"><div class="col-md-12 input_holder2sub middle-box"><div class=" well  well2sub" id="chkIndex_1" style=" overflow:auto; border-top:1px solid #c1c1c1;"><div class="col-md-4 col-sm-12 col-xs-12 form-group" style="padding: 0px;"><label style="border-right: 1px solid #c1c1c1 !important;">Material Type</label><select class="form-control selectAjaxOption select2  material_type_id select2" name="material_type[]" data-id="material_type" data-key="id" data-fieldname="name" tabindex="-1" aria-hidden="true" data-where="created_by_cid=' + logged_user + ' OR created_by_cid=0"  onchange="getMaterialName(event,this)" id="material_type"><option value="">Select Option</option></select></div><div class="col-md-4 col-sm-12 col-xs-12 form-group" style="padding: 0px;"><label style="border-right: 1px solid #c1c1c1 !important;">Material Name</label><select  class="materialNameId form-control col-md-2 col-xs-12 selectAjaxOption select2 Add_mat_onthe_spot" id="mat_name"  name="material_name[]" onchange="getUom_input(event,this);"><option value="">Select Option</option></select></div><div class="col-md-3 col-sm-12 col-xs-12 form-group" style="padding: 0px;"><label style="border-right: 1px solid #c1c1c1 !important;">Quantity</label><input style="border-right: 1px solid #c1c1c1 !important;" type="text" name="quantity[]" class="form-control col-md-7 col-xs-12 quantity" placeholder="Quantity" value=""></div><div class="col-md-1 col-sm-12 col-xs-12 form-group" style="padding: 0px;"><label style="border-right: 1px solid #c1c1c1 !important;">Action</label><div class="input-group-append1"><button class="btn edit-end-btn addmaterial2sub" type="button"><i class="fa fa-plus"></i></button></div></div></div></div></div></div></div><button class="btn btn-danger remove_machine" type="button"><i class="fa fa-minus"></i></button></div>');

            get_parameter_value();
            textareaWrap();
            subaddMaterialDetail2(well);
            subaddMaterialDetail3();
...
//get_parameter_value();
 function get_parameter_value() {
    $('.machine_name_id').on('change', function() {
        ...
        var ids = $(getPara).next().closest('.well').attr('data-id');
        console.log('ids', ids);
        $.ajax({
            type: 'POST',
            ...
            success: function(data) {
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    var val_1 = '';
                   if(ids == 'frst_div_1'){
                    parameter1 += '<input type="text"  name="parameter_' + ids + '[]" class="form-control col-md-7 col-xs-12" value="' + machine_parameter_1 + '" readonly>';
                    uom1 += '<input type="text"  name="uom_' + ids + '[]" class="form-control col-md-7 col-xs-12" value="' + uom_1 + '" readonly>';
                    value1 += '<input type="text"  name="value_' + ids + '[]" class="form-control col-md-7 col-xs-12" value="' + val_1 + '">';
                     }else{
					  alert('in else');
						    parameter1 +='<input type="text"  name="parameter_'+ids+'[]" class="form-control col-md-7 col-xs-12" value="'+ machine_parameter_1+'" readonly>';
							uom1 += '<input type="text"  name="uom_'+ids+'[]" class="form-control col-md-7 col-xs-12" value="'+uom_1+'" readonly>';
							value1 += '<input type="text"  name="value_'+ids+'[]" class="form-control col-md-7 col-xs-12" value="">';						  
					  }	


                    $(getPara).closest('.well').find('.parameter_name').html(parameter1);
                    $(getPara).closest('.well').find('.uom').html(uom1);
                    $(getPara).closest('.well').find('.value').html(value1);
                }
            }
        });
    });
...
function getMachineName(evt, t, selProcess = '', closestDiv = '') {
    //$('.machine_name_id').empty();
    $(this).parents().closest('input=[text]').find('.machine_name_id').empty();
    var option = $(t).find('option:selected');
    ...
    if (process_name_id != '') {
        ...
        //$('.machine_name_id').attr('data-where','process_name = '+process_name_id+' AND save_status=1');
        $('.machine_name_id').attr('data-where', "process LIKE '" + like + "' AND save_status=1");
        $('.machine_name_id').attr('data-id', 'add_machine');
        $('.machine_name_id').attr('data-key', 'id');
        $('.machine_name_id').attr('data-fieldname', 'machine_name');
...
function getUom(evt, t) {
    setTimeout(function() {
        ...
        $.ajax({
            type: "POST",
            url: site_url + 'maintenance/getMaterialDataById',
            data: {
                id: materialId
            },
            success: function(data) {
...
/* function addMaterial_inputDetail() {
    ...
    $(add_mat).click(function(e) {
        ...
        if (y < input) {
            ...
            $(this).closest('#' + div_id).parent().append('<div class="col col-md-12 chk_idd_input" id="sechIndexinput_' + y + '" ><div class="col-md-3 col-sm-12 col-xs-12 form-group"><select class="form-control selectAjaxOption select2 select2-hidden-accessible material_type_id select2" required="required" name="material_type_input_id_' + y + '[]" data-id="material_type" data-key="id" data-fieldname="name" tabindex="-1" aria-hidden="true" data-where="created_by_cid=' + logged_user + ' OR created_by_cid=0" onchange="getMaterialName(event,this)" id="material_type"><option value="">Select Option</option></select></div><div class="col-md-3 col-sm-12 col-xs-12 form-group"><select  class="materialNameId form-control col-md-2 col-xs-12 selectAjaxOption select2 Add_mat_onthe_spot" id="mat_name" required="required" name="material_input_name_' + y + '[]" onchange="getUom_input(event,this);"><option value="">Select Option</option></select></div><div class="col-md-3 col-sm-12 col-xs-12 form-group"><input type="text" id="qty" name="quantity_input_' + y + '[]" class="form-control col-md-7 col-xs-12  qty_input"  placeholder="Qty." value="" onkeyup="keyupFunction(event,this)" onchange="keyupFunction(event,this)" onkeypress="return float_validation(event, this.value)"></div><div class="col-md-2 col-sm-12 col-xs-12 form-group"><input type="text" name="uom_value_input1_' + y + '[]" class="form-control col-md-7 col-xs-12  uom_input" placeholder="uom." value="" readonly><input type="hidden" name="uom_value_input_' + y + '[]" class="uom_input_val" readonly value=""></div> <button class="btn  remv_inputss" type="button"><i class="fa fa-minus"></i></button>');
...
/* function addMaterial_outputDetail() {
    ...
    $(add_mat).click(function(e) {
        ...
        if (y < input) {
            y++;
            $(this).closest('#' + div_id).parent().append('<div class="col col-md-12 chk_idd_output" id="sechIndexoutput_' + y + '" ><div class="col-md-3 col-sm-12 col-xs-12 form-group"><select class="form-control selectAjaxOption select2 select2-hidden-accessible material_type_id select2" required="required" name="material_type_output_id_' + y + '[]" data-id="material_type" data-key="id" data-fieldname="name" tabindex="-1" aria-hidden="true" data-where="created_by_cid=' + logged_user + ' OR created_by_cid=0" onchange="getMaterialName(event,this)" id="material_type"><option value="">Select Option</option></select></div><div class="col-md-3 col-sm-12 col-xs-12 form-group"><select  class="materialNameId form-control col-md-2 col-xs-12 selectAjaxOption select2 Add_mat_onthe_spot" id="mat_name" required="required" name="material_output_name_' + y + '[]" onchange="getUom_output(event,this);"><option value="">Select Option</option></select></div><div class="col-md-3 col-sm-12 col-xs-12 form-group"><input type="text" id="qty" name="quantity_output_' + y + '[]" class="form-control col-md-7 col-xs-12  qty_output"  placeholder="Qty." value="" onkeyup="keyupFunction(event,this)" onchange="keyupFunction(event,this)" onkeypress="return float_validation(event, this.value)"></div>   <div class="col-md-2 col-sm-12 col-xs-12 form-group"><input type="text" name="uom_value_output1_' + y + '[]" class="form-control col-md-7 col-xs-12  uom_output" placeholder="uom." value="" readonly><input type="hidden" name="uom_value_output_' + y + '[]" class="uom_output_val" readonly value=""></div><button class="btn  remv_output" type="button"><i class="fa fa-minus"></i></button></div>');
...
/* function addMulitpleRow() {
    ...
    $(".maintable").on("click", ".addRow", function() {
        ...
        cols += '<div class="rTableCell"><label>Party Code</label><select class="form-control selectAjaxOption select2 select2-hidden-accessible party_code_cls" id ="party_code" name="party_code[' + mainTrIndex + '][' + i + ']_' + i + '" width="100%" tabindex="-1" aria-hidden="true" data-id="job_card" data-key="id" data-fieldname="party_code" tabindex="-1" aria-hidden="true" data-where="' + dataWhereJobCard + '"><option value="">Select Option</option></select></div>';

        cols += '<div class="rTableCell"><label>Job card Product Name</label><input id="job_card_product_name" class="form-control col-md-2 col-xs-12 job_card" name="job_card_product_name[' + mainTrIndex + '][' + i + ']_' + i + '" placeholder="Job Card Product Name" readonly  type="text" value=""><input type="hidden" id="job_card_product_id" class="form-control col-md-2 col-xs-12 job_card_product_id" name="job_card_product_id[' + mainTrIndex + '][' + i + ']_' + i + '" placeholder="Job Card Number" readonly  type="text" value=""></div>';
...
function AddRowPlan() {
    ...
    $(".rTable").on("click", ".addR", function() {
        ...
        cols += '<div class="rTableCell"><label>Job card product name</label><select class="form-control selectAjaxOption select2 select2-hidden-accessible" id ="job_no' + counter + '" name="job_card_product_name[' + mainTrIndex + '][' + i + ']_' + i + '"" width="100%" tabindex="-1" aria-hidden="true"  data-id="job_card" data-key="id" data-fieldname="job_card_product_name" tabindex="-1" aria-hidden="true" data-where="created_by_cid=' + dataWhereJobNo + '"></select></div>';
    ...
    $("div.maintable").on("click", "#btnDel", function(event) {
        $(this).closest(".rTableRow").remove();
        counter--;
    });
...
function get_jobcard_on_select() {
    $('.party_code_cls').on('change', 'select', function() {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                var job_card_product_name = dataObj[0]['job_card_product_name'];
...
function getDashboardCount(startDate = '', endDate = '') {
    ...
    $.ajax({
        ...
        url: site_url + 'maintenance/graphDashboardData/',
        dataType: 'json',
        type: 'POST',
        data: ajaxData,
        success: function(response) {
            ...
            $.each(response.getDashboardCount, function(key, value) {
                dashboardCountHtml += '<div class="animated flipInY col-lg-3 col-md-2 col-sm-6 col-xs-12"><div class="tile-stats"><div class="icon"><i class="' + value.icon + '"></i></div><div class="count">' + value.totalCount + '</div><h3>' + value.name + '</h3></div></div>';
            });
    ...
    $('.date').on('change', function() {
        if (window.location.href == site_url + 'production/production_data') {
            ...
            setTimeout(function() {
                ...
                //console.log("shift====>>>", $('input[type="radio"]:checked').val());
                    ...
                    success: function(result1) {
                        ...
                        if (result1 == 'Data of this date and shift already exist') {
                            ...
                            if (wagesData == '') {
                                ...
                                //var lenth2 = wagesPerpiece.length;
                                if (lenth != 0) {
                                    //console.log("ffgggggff");
                                    var k = 0;
                                    var i = 0;

                                    $('<div class="rTableRow mobile-view2"><div class="rTableHead  "><label>Select</label></div><div class="rTableHead"><label>Machine Name</label></div><div class="rTableHead"><label>Party Code</label></div><div class="rTableHead"><label>Job card Product Name</label></div><div class="rTableHead fix-width"><label>Workers</label></div>  <div class="rTableHead fix-width"><label>Hrs in Wages,% in per piece</label></div><div class="rTableHead"><label>Production Output</label></div><div class="rTableHead"><label>Labour costing</label></div><div class="rTableHead" ><label>Remarks</label></div><div class="rTableHead"></div></div>"').prependTo(".app_div");
            ...
            $.ajax({
                ...
                success: function(result1) {
                    if (result1 == 'Data of this date and shift already exist') {
                        ...
                        if (lenth != 0) {
                            $(".disablesubmitBtn").attr("disabled", false);
                            $(' <div class="rTableRow mobile-view2"><div class="rTableHead"><label>Machine Name<span class="required">*</span></label></div><div class="rTableHead"><label>Party Specification</label></div><div class="rTableHead"><label>Job card product name</label></div><div class="rTableHead"><label>Worker</label></div><div class="rTableHead"><label>Output</label></div><div class="rTableHead" ><label></label></div></div>"').prependTo(".app_div_planing");
                            var k = 0;
                            for (var j = 0; j < lenth; j++) {
...
/* function getQtyValue(evt, t) {
    ...
    var workOrderQty = $("#" + closestId + "").find('#qty').val();
    $.ajax({
        type: "POST",
            ...
            $(selectedMaterialId).each(function(index, value) {
                ...
                $(data).each(function(ind, val) {
                    ...
                    if (value == val.id) {
                        ...
                        console.log($('#chkIndex_' + index).find('.job_card').val(val.job_card_no));
...
function getBreakDownTargetVSActualListingGraph(startDate = '', endDate = '') {
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            //data = JSON.parse(result.getBreakDownTargetVSActualListingGraph);
            //console.log("responsedata",result.getBreakDownTargetVSActualListingGraph);
            var mychart = new Chart(ctx, {
                type: 'bar',
                data: result.getBreakDownTargetVSActualListingGraph,
                options: {
                    barValueSpacing: 20,
                    scales: {
                        yAxes: [{
...
function getBreakDownPurchanseGraph(startDate = '', endDate = '') {
    ...
    $.ajax({
        ...
        data: ajaxData,
        success: function(result) {
            var ctx = document.getElementById("BreakDownPurchase").getContext("2d");
            var mychart = new Chart(ctx, {
                type: 'bar',
                data: result.getBreakDownPurchanseGraph,
                options: {
                    barValueSpacing: 20,
                    scales: {
                        yAxes: [{
